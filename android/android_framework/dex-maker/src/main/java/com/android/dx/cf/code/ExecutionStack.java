...
public final class ExecutionStack extends MutabilityControl {
  ...
  /**
   * {@code non-null;} array specifying whether stack contents have entries in the local variable
   * table
   */
  ...
  /**
   * Gets the string form for a stack element. This is the same as {@code toString()} except that
   * {@code null} is converted to {@code "<invalid>"}.
   *
  ...
  /**
   * Replaces all the occurrences of the given uninitialized type in this stack with its initialized
   * equivalent.
   *
   ...
   * @throws SimException thrown if there is insufficient room on the stack for the value
   */
  ...
  /**
   * Peeks at the {@code n}th element down from the top of the stack. {@code n == 0} means to peek
   * at the top of the stack. Note that this will return {@code null} if the indicated element is
   * the deeper half of a category-2 value.
   *
  ...
  /**
   * Peeks at the {@code n}th element down from the top of the stack, returning whether or not it
   * has local info.
   *
   * @param n {@code >= 0;} which element to peek at
  ...
  /**
   * Peeks at the {@code n}th element down from the top of the stack, returning the type per se, as
   * opposed to the <i>type-bearer</i>. This method is just a convenient shorthand for {@code
   * peek(n).getType()}.
   *
   * @see #peek
   */
  ...
  /**
   * Changes an element already on a stack. This method is useful in limited contexts, particularly
   * when merging two instances. As such, it places the following restriction on its behavior: You
   * may only replace values with other values of the same category.
   *
   * @param n {@code >= 0;} which element to change, where {@code 0} is the top element of the stack
   * @param type {@code non-null;} type of the new value
   * @throws SimException thrown if {@code n >= size()} or the action is otherwise prohibited
   */
  ...
  /**
   * Merges this stack with another stack. A new instance is returned if this merge results in a
   * change. If no change results, this instance is returned. See {@link
   * Merger#mergeStack(ExecutionStack, ExecutionStack) Merger.mergeStack()}
   *
