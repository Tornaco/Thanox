...
public final class Ropper {
  ...
  /**
   * {@code non-null;} list of subroutine-nest labels (See {@link Frame#getSubroutines} associated
   * with each result block. Parallel to {@link Ropper#result}.
   */
  private final ArrayList<IntList> resultSubroutines;

  /**
   * {@code non-null;} for each block (by label) that is used as an exception handler in the input,
   * the exception handling info in Rop.
   */
   ...
   * @param methods {@code non-null;} list of methods defined by the class that defines {@code
   *     method}.
   * @param dexOptions {@code non-null;} options for dex output
   */
  private Ropper(
      ConcreteMethod method, TranslationAdvice advice, MethodList methods, DexOptions dexOptions) {
    if (method == null) {
   ...
   */
  public static RopMethod convert(
      ConcreteMethod method, TranslationAdvice advice, MethodList methods, DexOptions dexOptions) {
    try {
  ...
  /**
   * Gets the first (lowest) register number to use as the temporary area when unwinding stack
   * manipulation ops.
   *
   ...
   *
   * <ul>
   *   <li>[0, maxLabel[ are the labels of the blocks directly corresponding to the input bytecode.
   *   <li>[maxLabel, maxLabel + method.getCatches().size()[ are reserved for exception setup
   *       blocks.
   *   <li>[maxLabel + method.getCatches().size(), maxLabel + method.getCatches().size() +
   *       SPECIAL_LABEL_COUNT[ are reserved for special blocks, ie param assignement, return and
   *       synch blocks.
   *   <li>[maxLabel method.getCatches().size() + SPECIAL_LABEL_COUNT, getAvailableLabel()[ assigned
   *       labels. Note that some of the assigned labels may not be used any more if they were
   *       assigned to a block that was deleted since.
   * </ul>
   *
   * @return {@code >= 0;} an available label with the guaranty that all greater labels are also
   *     available.
   */
  ...
  /**
   * Gets the total number of registers used for "normal" purposes (i.e., for the straightforward
   * translation from the original Java).
   *
  ...
  /**
   * Searches {@link #result} for a block with the given label. Returns its index if found, or
   * returns {@code -1} if there is no such block.
   *
   * @param label the label to look for
   * @return {@code >= -1;} the index for the block with the given label or {@code -1} if there is
   *     no such block
   */
  ...
  /**
   * Searches {@link #result} for a block with the given label. Returns it if found, or throws an
   * exception if there is no such block.
   *
   * @param label the label to look for
   ...
   * @param subroutines {@code non-null;} subroutine label list as described in {@link
  ...
  /**
   * Adds or replace a block in the output result. If this is a replacement, then any extra blocks
   * that got added with the original get removed as a result of calling this method.
   *
   * @param block {@code non-null;} the block to add or replace
   * @param subroutines {@code non-null;} subroutine label list as described in {@link
   *     Frame#getSubroutines}
   * @return {@code true} if the block was replaced or {@code false} if it was added for the first
   *     time
   */
  ...
  /**
   * Helper for {@link #addOrReplaceBlock} which recursively removes the given block and all blocks
   * that are (direct and indirect) successors of it whose labels indicate that they are not in the
   ...
   * @param workSet {@code non-null;} bits representing work to do, which this method may add to
   */
  private void processBlock(ByteBlock block, Frame frame, int[] workSet) {
     ...
     */
    for (
    /*extraBlockCount*/ ; extraBlockCount > 0; extraBlockCount--) {
      /*
   ...
   * @param pred predecessor label; must be {@code >= 0} when {@code label} is a subroutine start
   *     block and calledSubroutine is non-null. Otherwise, may be -1.
   * @param calledSubroutine {@code null-ok;} a Subroutine instance if {@code label} is the first
   *     block in a subroutine.
   * @param frame {@code non-null;} new frame for the labelled block
   * @param workSet {@code non-null;} bits representing work to do, which this method may add to
   */
  private void mergeAndWorkAsNecessary(
      int label, int pred, Subroutine calledSubroutine, Frame frame, int[] workSet) {
  ...
  private void addSetupBlocks() {
    ...
    for (int i = 0; i < sz; i++) {
      ...
      Insn insn =
          new PlainCstInsn(
              Rops.opMoveParam(one), pos, result, RegisterSpecList.EMPTY, CstInteger.make(at));
      insns.set(i, insn);
      at += one.getCategory();
    }
    ...
    BasicBlock bb =
        new BasicBlock(
            getSpecialLabel(PARAM_ASSIGNMENT), insns, IntList.makeImmutable(label), label);
    ...
    if (synch) {
      ...
      if (isStatic()) {
        ...
        insn =
            new PlainCstInsn(
                Rops.MOVE_PARAM_OBJECT, pos, synchReg, RegisterSpecList.EMPTY, CstInteger.VALUE_0);
        insns.set(0, insn);
      ...
      if (isStatic()) {
        insns.set(
            0,
            new PlainInsn(
                Rops.opMoveResultPseudo(synchReg), pos, synchReg, RegisterSpecList.EMPTY));
      }

      insn =
          new ThrowingInsn(
              Rops.MONITOR_ENTER, pos, RegisterSpecList.make(synchReg), StdTypeList.EMPTY);
  ...
  /**
   * Constructs and adds the return block, if necessary. The return block merely contains an
   * appropriate {@code return} instruction.
   */
  ...
  /**
   * Constructs and adds, if necessary, the catch-all exception handler block to deal with unwinding
   * the lock taken on entry to a synchronized method.
   */
  private void addSynchExceptionHandlerBlock() {
    if (!synchNeedsExceptionHandler) {
      /*
       * The method being converted either isn't synchronized or
       * can't possibly throw exceptions in its boot body, so
    ...
    insn =
        new ThrowingInsn(
            Rops.MONITOR_EXIT, pos, RegisterSpecList.make(getSynchReg()), StdTypeList.EMPTY);
    insns.set(1, insn);
    insns.setImmutable();
    ...
    bb =
        new BasicBlock(
            getSpecialLabel(SYNCH_CATCH_1), insns, IntList.makeImmutable(label2), label2);
    addBlock(bb, IntList.EMPTY);
  ...
  /**
   * Creates the exception handler setup blocks. "maxLocals" below is because that's the register
   * number corresponding to the sole element on a one-deep stack (which is the situation at the
   * start of an exception handler block).
   */
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (catches != null) {
        for (ExceptionHandlerSetup one : catches.getSetups()) {
          ...
          Insn insn =
              new PlainInsn(
                  Rops.opMoveException(one.getCaughtType()),
                  pos,
  ...
  private void inlineSubroutines() {
    ...
    for (int i = 0; i < sz; i++) {
      ...
      new SubroutineInliner(new LabelAllocator(getAvailableLabel()), labelToSubroutines)
  ...
  /**
   * Deletes all blocks that cannot be reached. This is run to delete original subroutine blocks
  ...
  private void deleteUnreachableBlocks() {
    ...
    forEachNonSubBlockDepthFirst(
        getSpecialLabel(PARAM_ASSIGNMENT),
        new BasicBlock.Visitor() {
   ...
   * @return {@code null-ok;} found subroutine or {@code null} if none was found
   */
  ...
  /**
   * Removes all {@code move-return-address} instructions, returning a new {@code InsnList} if
   * necessary. The {@code move-return-address} insns are dead code after subroutines have been
   * inlined.
   *
   * @param insns {@code InsnList} that may contain {@code move-return-address} insns
  ...
  /**
   * Visits each block once in depth-first successor order, ignoring {@code jsr} targets. Worker for
   * {@link #forEachNonSubBlockDepthFirst}.
   *
  ...
  /**
   * Allocates labels, without requiring previously allocated labels to have been added to the
   * blocks list.
   */
  ...
  private class Subroutine {
     ...
     *
     * @param startBlock First block of the subroutine.
     * @param retBlock one of the ret blocks (final blocks) of this subroutine.
     */
    ...
    /**
     * Generates a list of subroutine successors. Note: successor blocks could be listed more than
     * once. This is ok, because this successor list (and the block it's associated with) will be
     * copied and inlined before we leave the ropper. Redundent successors will result in redundent
     * (no-op) merges.
     *
    ...
    IntList getSuccessors() {
      ...
      for (int label = callerBlocks.nextSetBit(0);
          label >= 0;
    ...
    /**
     * Merges the specified frame into this subroutine's successors, setting {@code workSet} as
     * appropriate. To be called with the frame of a subroutine ret block.
     *
  ...
  /** Inlines a subroutine. Start by calling {@link #inlineSubroutineCalledFrom}. */
  private class SubroutineInliner {
    /** maps original label to the label that will be used by the inlined version */
    ...
    /**
     * A mapping, indexed by label, to subroutine nesting list. The subroutine nest list is as
     * returned by {@link Frame#getSubroutines}.
     */
    ...
    SubroutineInliner(final LabelAllocator labelAllocator, ArrayList<IntList> labelToSubroutines) {
    ...
    private void copyBlock(int origLabel, int newLabel) {
      ...
      if (isSubroutineCaller(origBlock)) {
        ...
        successors =
            IntList.makeImmutable(mapOrAllocateLabel(origSuccessors.get(0)), origSuccessors.get(1));
     ...
     * @param subroutineStart {@code >= 0;} a subroutine as identified by the label of its start
     *     block
    ...
    private boolean involvedInSubroutine(int label, int subroutineStart) {
      ...
      return (subroutinesList != null
          && subroutinesList.size() > 0
          && subroutinesList.top() == subroutineStart);
    }

    /**
     * Maps the label of a pre-copied block to the label of the inlined block, allocating a new
     * label and adding it to the worklist if necessary. If the origLabel is a "special" label, it
     * is returned exactly and not scheduled for duplication: copying never proceeds past a special
     * label, which likely is the function return block or an immediate predecessor.
     *
