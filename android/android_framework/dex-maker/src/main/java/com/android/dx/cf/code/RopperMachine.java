...
/*package*/ final class RopperMachine extends ValueAwareMachine {
  ...
  /** {@code non-null;} method constant for use in converting {@code multianewarray} instructions */
  private static final CstMethodRef MULTIANEWARRAY_METHOD =
      new CstMethodRef(
          ARRAY_REFLECT_TYPE,
          new CstNat(
              new CstString("newInstance"),
              new CstString("(Ljava/lang/Class;[I)" + "Ljava/lang/Object;")));
  ...
  /**
   * If non-null, the ReturnAddress that was used by the terminating ret instruction. If null, there
   * was no ret instruction encountered.
   */
  private ReturnAddress returnAddress;

  /** {@code null-ok;} the appropriate {@code return} op or {@code null} if it is not yet known */
  private Rop returnOp;

  /**
   * {@code null-ok;} the source position for the return block or {@code null} if it is not yet
   * known
   */
  ...
  public RopperMachine(
      Ropper ropper, ConcreteMethod method, TranslationAdvice advice, MethodList methods) {
  ...
  /**
   * Gets the instructions array. It is shared and gets modified by subsequent calls to this
   * instance.
   *
  ...
  /**
   * Gets ready to start working on a new block. This will clear the {@link #insns} list, set {@link
   * #catches}, reset whether it has been used, reset whether the block contains a {@code return},
   * and reset {@link #primarySuccessorIndex}.
   */
  ...
  /**
   * Gets whether {@link #catches} was used. This indicates that the last instruction in the block
   * is one of the ones that can throw.
   *
  ...
  /**
   * Gets the primary successor index. This is the index into the successors list where the primary
   * may be found or {@code -1} if there are successors but no primary successor. This may return
   * something other than {@code -1} in the case of an instruction with no successors at all
  ...
  /**
   * Gets how many extra blocks will be needed to represent the block currently being translated.
   * Each extra block should consist of one instruction from the end of the original block.
   *
  ...
  /**
   * @return true if at least one of the insn processed since the last call to startBlock() can
   *     throw.
   */
  ...
  /** @return true if a JSR has ben encountered since the last call to startBlock() */
  public boolean hasJsr() {
    return hasJsr;
  }

  /**
   * @return {@code true} if a {@code ret} has ben encountered since the last call to {@code
   *     startBlock()}
   */
  ...
  /**
   * @return {@code null-ok;} return address of a {@code ret} instruction if encountered since last
   *     call to startBlock(). {@code null} if no ret instruction encountered.
   */
  ...
  public void run(Frame frame, int offset, int opcode) {
    ...
    if (destCount == 0) {
       ...
       * then move stuff back down onto the boot "stack" in the
    ...
    if (opcode == ByteOps.MULTIANEWARRAY) {
      ...
      if (componentType.isPrimitive()) {
        ...
        insn =
            new ThrowingCstInsn(
                Rops.GET_STATIC_OBJECT, pos, RegisterSpecList.EMPTY, catches, typeField);
      } else {
        /*
      ...
      insn =
          new ThrowingCstInsn(
              ...
              pos,
              RegisterSpecList.make(classReg, dimsReg),
              catches,
    ...
    if (dest != null && rop.isCallLike()) {
      ...
      moveResult =
          new PlainInsn(
              Rops.opMoveResultPseudo(dest.getTypeBearer()), pos, dest, RegisterSpecList.EMPTY);
    ...
    if (ropOpcode == RegOps.NEW_ARRAY) {
      ...
      if ((lastType.isConstant() || firstType.isConstant())
          && advice.hasConstantOperation(rop, sources.get(0), sources.get(1))) {
    ...
    if (initValues != null) {
      extraBlockCount++;
      insn =
          new FillArrayDataInsn(
              Rops.FILL_ARRAY_DATA,
              pos,
              RegisterSpecList.make(moveResult.getResult()),
              initValues,
              cst);
      insns.add(insn);
    }
   ...
   * @param stackPointer {@code >= 0;} the stack pointer after the instruction's arguments have been
   *     popped
      ...
      case ByteOps.INVOKESPECIAL:
        {
           ...
           */
          CstMethodRef ref = (CstMethodRef) cst;
          if (ref.isInstanceInit() || (ref.getDefiningClass().equals(method.getDefiningClass()))) {
            return RegOps.INVOKE_DIRECT;
          }
  ...
  private Insn makeInvokePolymorphicInsn(
      Rop rop, SourcePosition pos, RegisterSpecList sources, TypeList catches, Constant cst) {
