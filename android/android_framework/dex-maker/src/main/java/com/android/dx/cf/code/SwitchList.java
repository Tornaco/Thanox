...
/**
 * List of (value, target) mappings representing the choices of a {@code tableswitch} or {@code
 * lookupswitch} instruction. It also holds the default target for the switch.
 */
public final class SwitchList extends MutabilityControl {
  ...
  /**
   * {@code non-null;} list of targets corresponding to the test values; there is always one extra
   * element in the target list, to hold the default target
   */
  ...
  /**
   * Gets the indicated target. Asking for the target at {@code size()} returns the default target.
   *
   * @param n {@code >= 0, <= size();} which index
  ...
  /**
   * Gets the default target. This is just a shorthand for {@code getTarget(size())}.
   *
  ...
  /**
   * Sets the default target. It is only valid to call this method when all the non-default elements
   * have been set.
   *
   * @param target {@code >= 0;} the absolute (not relative) default target address
   */
  ...
  /**
   * Gets the list of all targets. This includes one extra element at the end of the list, which
   * holds the default target.
   *
   ...
   * This is only valid after the instance is fully populated, including the default target
   * (naturally).
   */
