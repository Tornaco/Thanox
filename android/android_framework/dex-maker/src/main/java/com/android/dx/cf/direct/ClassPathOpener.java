...
/**
 * Opens all the class files found in a class path element. Path elements can point to class files,
 * {jar,zip,apk} files, or directories containing class files.
 */
public class ClassPathOpener {
  ...
  /**
   * If true, sort such that classes appear before their inner classes and "package-info" occurs
   * before all other classes in that package.
   */
   ...
   * @param sort if true, sort such that classes appear before their inner classes and
  ...
  /**
   * Sorts java class names such that outer classes preceed their inner classes and "package-info"
   * preceeds all other classes in its package.
   *
   ...
   *
   * @return the OR of all return values from {@code Consumer.processFileBytes()}.
   */
   ...
   * @param topLevel whether this is a top-level file (that is, specified directly on the
   *     commandline)
   ...
   * @param topLevel whether this is a top-level directory (that is, specified directly on the
   *     commandline)
   * @return whether any processing actually happened
   */
  ...
  public interface Consumer {
     ...
     *
     * @param name {@code non-null;} filename of element. May not be a valid filesystem path.
     * @param lastModified milliseconds since 1970-Jan-1 00:00:00 GMT
     * @param bytes {@code non-null;} file data
     * @return true on success. Result is or'd with all other results from {@code processFileBytes}
     *     and returned to the caller of {@code process()}.
     */
    boolean processFileBytes(String name, long lastModified, byte[] bytes);

    /**
     * Informs consumer that an exception occurred while processing this path element. Processing
