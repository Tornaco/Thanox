...
public class DirectClassFile implements ClassFile {
   ...
   *
   * <p>See http://en.wikipedia.org/wiki/Java_class_file for an up-to-date list of version numbers.
   * Currently known (taken from that table) are:
   *
   * <p>Java SE 9 = 53 (0x35 hex), Java SE 8 = 52 (0x34 hex), Java SE 7 = 51 (0x33 hex), Java SE 6.0
   * = 50 (0x32 hex), Java SE 5.0 = 49 (0x31 hex), JDK 1.4 = 48 (0x30 hex), JDK 1.3 = 47 (0x2F hex),
   * JDK 1.2 = 46 (0x2E hex), JDK 1.1 = 45 (0x2D hex).
   *
   * <p>Valid ranges are typically of the form "A.0 through B.C inclusive" where A <= B and C >= 0,
   ...
   *
   * <p>Note: if you change this, please change "java.class.version" in System.java.
   */
  ...
  /**
   * whether to be strict about parsing; if {@code false}, this avoids doing checks that only exist
   * for purposes of verification (such as magic number matching and path-package consistency
   * checking)
   */
  private final boolean strictParse;

  /**
   * {@code null-ok;} the constant pool; only ever {@code null} before the constant pool is
   * successfully parsed
   */
  private StdConstantPool pool;

  /**
   * the class file field {@code access_flags}; will be {@code -1} before the file is successfully
   * parsed
   */
  private int accessFlags;

  /**
   * {@code null-ok;} the class file field {@code this_class}, interpreted as a type constant; only
   * ever {@code null} before the file is successfully parsed
   */
  private CstType thisClass;

  /**
   * {@code null-ok;} the class file field {@code super_class}, interpreted as a type constant if
   * non-zero
   */
  private CstType superClass;

  /**
   * {@code null-ok;} the class file field {@code interfaces}; only ever {@code null} before the
   * file is successfully parsed
   */
  private TypeList interfaces;

  /**
   * {@code null-ok;} the class file field {@code fields}; only ever {@code null} before the file is
   * successfully parsed
   */
  private FieldList fields;

  /**
   * {@code null-ok;} the class file field {@code methods}; only ever {@code null} before the file
   * is successfully parsed
   */
  private MethodList methods;

  /**
   * {@code null-ok;} the class file field {@code attributes}; only ever {@code null} before the
   * file is successfully parsed
   */
   ...
   *     specification
   * @param strictParse whether to be strict about parsing; if {@code false}, this avoids doing
   *     checks that only exist for purposes of verification (such as magic number matching and
  ...
  /**
   * Returns the string form of an object or {@code "(none)"} (rather than {@code "null"}) for
   * {@code null}.
   *
  ...
  /**
   * Constructs and returns an instance of {@link TypeList} whose data comes from the bytes of this
   * instance, interpreted as a list of constant pool indices for classes, which are in turn
   * translated to type constants. Instance construction will fail if any of the (alleged) indices
   * turn out not to refer to constant pool entries of type {@code Class}.
   *
  ...
  private void parse0() {
    ...
    if (strictParse) {
      ...
      if (!isGoodVersion(getMinorVersion0(), getMajorVersion0())) {
        throw new ParseException(
            "unsupported class file version " + getMajorVersion0() + "." + getMinorVersion0());
      }
    }
    ...
    if (strictParse) {
      ...
      if (!(filePath.endsWith(".class")
        ...
        throw new ParseException(
            "class name (" + thisClassName + ") does not match path (" + filePath + ")");
      }
    }
    ...
    AttributeListParser alParser =
        new AttributeListParser(this, AttributeFactory.CTX_CLASS, at, attributeFactory);
  ...
  /**
   * Implementation of {@link TypeList} whose data comes directly from the bytes of an instance of
   * this (outer) class, interpreted as a list of constant pool indices for classes which are in
   * turn returned as type constants. Instance construction will fail if any of the (alleged)
   * indices turn out not to refer to constant pool entries of type {@code Class}.
   */
  private static class DcfTypeList implements TypeList {
     ...
     */
    public DcfTypeList(
        ByteArray bytes, int offset, int size, StdConstantPool pool, ParseObserver observer) {
      if (size < 0) {
