...
/**
 * Utility methods that translate various classfile attributes into forms suitable for use in
 * creating {@code dex} files.
 */
/*package*/ class AttributeTranslator {
  ...
  /**
   * Gets the annotations out of a given {@link AttributeList}. This combines both visible and
   * invisible annotations into a single result set and also adds in a system annotation for the
  ...
  /**
   * Gets the annotations out of a given class, similar to {@link #getAnnotations}, also including
   * annotations for translations of class-level attributes {@code EnclosingMethod} and {@code
   * InnerClasses}, if present. Additionally, if the class is an annotation class, then this also
   * includes a representation of all the {@code AnnotationDefault} values.
   *
  ...
  public static Annotations getClassAnnotations(DirectClassFile cf, CfOptions args) {
    ...
    try {
      Annotations innerClassAnnotations =
          translateInnerClasses(thisClass, attribs, enclosingMethod == null);
  ...
  /**
   * Gets the annotations out of a given method, similar to {@link #getAnnotations}, also including
   * an annotation for the translation of the method-specific attribute {@code Exceptions}.
   *
   * @param method {@code non-null;} the method in question
   * @return {@code non-null;} the set of annotations, which may be empty
   */
  ...
  /**
   * Gets the {@code Signature} attribute out of a given {@link AttributeList}, if any, translating
   * it to an annotation.
   *
   * @param attribs {@code non-null;} the attributes list to search in
   * @return {@code null-ok;} the converted {@code Signature} annotation, if there was an attribute
   *     to translate
   */
  ...
  private static Annotation getSourceDebugExtension(AttributeList attribs) {
    AttSourceDebugExtension extension =
        (AttSourceDebugExtension) attribs.findFirst(AttSourceDebugExtension.ATTRIBUTE_NAME);
  ...
  /**
   * Gets the {@code EnclosingMethod} attribute out of a given {@link AttributeList}, if any,
   * translating it to an annotation. If the class really has an enclosing method, this returns an
   * {@code EnclosingMethod} annotation; if not, this returns an {@code EnclosingClass} annotation.
   *
   * @param attribs {@code non-null;} the attributes list to search in
   * @return {@code null-ok;} the converted {@code EnclosingMethod} or {@code EnclosingClass}
   *     annotation, if there was an attribute to translate
   */
  private static Annotation translateEnclosingMethod(AttributeList attribs) {
    AttEnclosingMethod enclosingMethod =
        (AttEnclosingMethod) attribs.findFirst(AttEnclosingMethod.ATTRIBUTE_NAME);
  ...
  /**
   * Gets the {@code InnerClasses} attribute out of a given {@link AttributeList}, if any,
   * translating it to one or more of an {@code InnerClass}, {@code EnclosingClass}, or {@code
   ...
   * @return {@code null-ok;} the converted list of annotations, if there was an attribute to
   *     translate
   */
  private static Annotations translateInnerClasses(
      CstType thisClass, AttributeList attribs, boolean needEnclosingClass) {
    AttInnerClasses innerClasses =
        (AttInnerClasses) attribs.findFirst(AttInnerClasses.ATTRIBUTE_NAME);
    ...
    if (foundThisClass != null) {
      result.add(
          AnnotationUtils.makeInnerClass(
              foundThisClass.getInnerName(), foundThisClass.getAccessFlags()));
  ...
  /**
   * Gets the parameter annotations out of a given method. This combines both visible and invisible
   * annotations into a single result set.
   *
   * @param method {@code non-null;} the method in question
  ...
  public static AnnotationsList getParameterAnnotations(Method method) {
    ...
    AttRuntimeVisibleParameterAnnotations visible =
        (AttRuntimeVisibleParameterAnnotations)
            attribs.findFirst(AttRuntimeVisibleParameterAnnotations.ATTRIBUTE_NAME);
    AttRuntimeInvisibleParameterAnnotations invisible =
        (AttRuntimeInvisibleParameterAnnotations)
            attribs.findFirst(AttRuntimeInvisibleParameterAnnotations.ATTRIBUTE_NAME);
    ...
    return AnnotationsList.combine(
        visible.getParameterAnnotations(), invisible.getParameterAnnotations());
  }

  /**
   * Gets the {@code AnnotationDefault} attributes out of a given class, if any, reforming them as
   * an {@code AnnotationDefault} annotation.
   *
   * @param cf {@code non-null;} the class in question
   * @return {@code null-ok;} an appropriately-constructed {@code AnnotationDefault} annotation, if
   *     there were any annotation defaults in the class, or {@code null} if not
   */
  private static Annotation translateAnnotationDefaults(DirectClassFile cf) {
    ...
    for (int i = 0; i < sz; i++) {
      ...
      AttAnnotationDefault oneDefault =
          (AttAnnotationDefault) attribs.findFirst(AttAnnotationDefault.ATTRIBUTE_NAME);
