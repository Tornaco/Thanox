...
/**
 * Static method that turns {@code byte[]}s containing Java classfiles into {@link ClassDefItem}
 * instances.
 */
public class CfTranslator {
  ...
  /**
   * Takes a {@code byte[]}, interprets it as a Java classfile, and translates it into a {@link
   * ClassDefItem}.
   *
  ...
  /**
   * Performs the boot act of translation. This method is separated from {@link #translate} just to
  ...
  private static ClassDefItem translate0(
    ...
    context.optimizerOptions.loadOptimizeLists(
    ...
    for (int i = 0; i < constantPoolSize; i++) {
      ...
      if (constant instanceof CstMethodRef) {
        ...
        CstCallSite callSite =
            CstCallSite.make(
                bootstrapMethod.getBootstrapMethodHandle(),
  ...
  /**
   * Helper for {@link #processFields}, which translates constants into more specific types if
   * necessary.
   *
      ...
      try {
        ...
        if (isNative || isAbstract) {
          ...
          if (cfOptions.optimize && context.optimizerOptions.shouldOptimize(canonicalName)) {
            ...
            if (DEBUG) {
              context.optimizerOptions.compareOptimizerStep(
