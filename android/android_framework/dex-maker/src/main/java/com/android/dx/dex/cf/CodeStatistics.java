...
public final class CodeStatistics {
   ...
   * Only valid if args.statistics is true.
   */
  public int dexRunningDeltaRegisters = 0;

  /**
   * running sum of the number of dex-form insns (actually code units) added/removed in SSA form by
   * the optimizer. Only valid if args.statistics is true.
   */
  ...
  public void updateDexStatistics(DalvCode nonOptCode, DalvCode code) {
    ...
    dexRunningDeltaRegisters +=
        (code.getInsns().getRegistersSize() - nonOptCode.getInsns().getRegistersSize());
  ...
  public void dumpStatistics(PrintStream out) {
    out.printf(
        "Optimizer Delta Rop Insns: %d total: %d " + "(%.2f%%) Delta Registers: %d\n",
        runningDeltaInsns,
        runningTotalInsns,
        (100.0 * (((float) runningDeltaInsns) / (runningTotalInsns + Math.abs(runningDeltaInsns)))),
        runningDeltaRegisters);

    out.printf(
        "Optimizer Delta Dex Insns: Insns: %d total: %d " + "(%.2f%%) Delta Registers: %d\n",
        dexRunningDeltaInsns,
        dexRunningTotalInsns,
        (100.0
            * (((float) dexRunningDeltaInsns)
                / (dexRunningTotalInsns + Math.abs(dexRunningDeltaInsns)))),
        dexRunningDeltaRegisters);
