...
/**
 * Container for all the pieces of a concrete method. Each instance corresponds to a {@code code}
 * structure in a {@code .dex} file.
 */
public final class DalvCode {
  ...
  /**
   * {@code null-ok;} the instruction list, ready for final processing; nulled out in {@link
   ...
   *     PositionList}
   * @param unprocessedInsns {@code non-null;} the instruction list, ready for final processing
   * @param unprocessedCatches {@code non-null;} unprocessed catch (exception handler) table
   */
  public DalvCode(
      int positionInfo, OutputFinisher unprocessedInsns, CatchBuilder unprocessedCatches) {
   ...
   * This must be called before {@link #getInsns}.
   *
   ...
   *
   * @return {@code true} iff this instance has any local variable data to represent
   */
  ...
  public interface AssignIndicesCallback {
     ...
     * @return {@code >= -1;} the index or {@code -1} if the constant shouldn't actually be reified
     *     with an index
     */
