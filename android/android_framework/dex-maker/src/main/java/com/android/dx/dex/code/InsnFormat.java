...
/**
 * Base class for all instruction format handlers. Instruction format handlers know how to translate
 * {@link DalvInsn} instances into streams of code units, as well as human-oriented listing strings
 * representing such translations.
 */
public abstract class InsnFormat {
  /**
   * flag to enable/disable the new extended opcode formats; meant as a temporary measure until VM
   * support for the salient opcodes is added. TODO: Remove this declaration when the VM can deal.
   */
   ...
   *
   * @param list {@code non-null;} the list of registers (which must be sequential)
   * @return {@code non-null;} the string form
   */
   ...
   *
   * @param value the value
   * @param width the width of the constant, in bits (used for displaying the uninterpreted bits;
   *     one of: {@code 4 8 16 32 64}
  ...
  /**
   * Helper method to determine if a list of registers are sequential, including degenerate cases
  ...
  /**
   * Helper method to combine an opcode and a second byte of data into the appropriate form for
   * emitting into a code buffer.
   *
  ...
  /**
   * Helper method to get an extended (16-bit) opcode out of an instruction, returning it as a code
   * unit. The opcode <i>must</i> be an extended opcode.
   *
   * @param insn {@code non-null;} the instruction containing the extended opcode
  ...
  protected static void write(
      AnnotatedOutput out, short c0, short c1, short c2, short c3, short c4) {
    out.writeShort(c0);
    out.writeShort(c1);
    out.writeShort(c2);
    out.writeShort(c3);
  ...
  /**
   * Writes three code units to the given output destination, where the second and third are
   * represented as single <code>int</code> and emitted in little-endian order.
   *
   * @param out {@code non-null;} where to write to
   * @param c0 code unit to write
  ...
  /**
   * Writes four code units to the given output destination, where the second and third are
   * represented as single <code>int</code> and emitted in little-endian order.
   *
   * @param out {@code non-null;} where to write to
   * @param c0 code unit to write
  ...
  /**
   * Writes five code units to the given output destination, where the second and third are
   * represented as single <code>int</code> and emitted in little-endian order.
   *
   * @param out {@code non-null;} where to write to
   * @param c0 code unit to write
  ...
  /**
   * Writes five code units to the given output destination, where the second through fifth are
   * represented as single <code>long</code> and emitted in little-endian order.
   *
   * @param out {@code non-null;} where to write to
   * @param c0 code unit to write
   ...
   * The instruction must be of this instance's format for proper operation.
   *
   * @param insn {@code non-null;} the instruction
  ...
  /**
   * Returns the string form of the arguments to the given instruction. The instruction must be of
   * this instance's format. If the instruction has no arguments, then the result should be {@code
   * ""}, not {@code null}.
   *
  ...
  /**
   * Returns the associated comment for the given instruction, if any. The instruction must be of
   * this instance's format. If the instruction has no comment, then the result should be {@code
   * ""}, not {@code null}.
   *
   * <p>Subclasses must override this method.
   *
  ...
  /**
   * Gets the code size of instructions that use this format. The size is a number of 16-bit code
   * units, not bytes. This should throw an exception if this format is of variable size.
   *
   ...
   * This includes such things as counting register arguments, checking register ranges, and making
   * sure that additional arguments are of appropriate types and are in-range. If this format has a
   * branch target but the instruction's branch offset is unknown, this method will simply not check
   * the offset.
   *
   * <p>Subclasses must override this method.
   *
   * @param insn {@code non-null;} the instruction to check
   * @return {@code true} iff the instruction's arguments are appropriate for this instance, or
   *     {@code false} if not
   */
   ...
   *
   * <p>The default implementation of this method always returns an empty BitSet. Subclasses must
   * override this method if they have registers.
   *
   * @param insn {@code non-null;} the instruction to check
   * @return {@code non-null;} a BitSet flagging registers in the register list that are compatible
   *     to this format
   */
  ...
  /**
   * Returns whether or not the given instruction's branch offset will fit in this instance's
   * format. This always returns {@code false} for formats that don't include a branch offset.
   *
   * <p>The default implementation of this method always returns {@code false}. Subclasses must
   * override this method if they include branch offsets.
   *
   * @param insn {@code non-null;} the instruction to check
   * @return {@code true} iff the instruction's branch offset is appropriate for this instance, or
   *     {@code false} if not
   */
  ...
  /**
   * Writes the code units for the given instruction to the given output destination. The
   * instruction must be of this instance's format.
   *
   * <p>Subclasses must override this method.
   *
