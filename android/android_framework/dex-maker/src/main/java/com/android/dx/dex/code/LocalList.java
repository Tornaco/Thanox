...
/**
 * List of local variables. Each local variable entry indicates a range of code which it is valid
 * for, a register number, a name, and a type.
 */
public final class LocalList extends FixedSizeList {
  ...
  /**
   * Constructs an instance for the given method, based on the given block order and intermediate
   * local information.
   *
  ...
  /**
   * Debugging helper that verifies the constraint that a list doesn't contain any redundant local
   * starts and that local ends that are due to replacements are properly annotated.
   */
  ...
  /**
   * Gets the element at the given index. It is an error to call this with the index for an element
   * which was never set; if you do that, this will throw {@code NullPointerException}.
   *
  ...
  public static class Entry implements Comparable<Entry> {
    ...
    /**
     * Compares by (in priority order) address, end then start disposition (variants of end are all
     * consistered equivalent), and spec.
     *
    ...
    /**
     * Gets whether this is a local start. This is just shorthand for {@code getDisposition() ==
     * Disposition.START}.
     *
  ...
  public static class MakeState {
    ...
    /** {@code >= 0;} running count of nulled result entries, to help with sizing the final list */
    ...
    /**
     * Converts a given spec into the form acceptable for use in a local list. This, in particular,
     * transforms the "known null" type into simply {@code Object}. This method needs to be called
     * for any spec that is on its way into a locals list.
     *
     * <p>This isn't necessarily the cleanest way to achieve the goal of not representing known
     * nulls in a locals list, but it gets the job done.
     *
     * @param orig {@code null-ok;} the original spec
     * @return {@code null-ok;} an appropriately modified spec, or the original if nothing needs to
     *     be done
     */
    ...
    /**
     * Sets the local state at the given address to the given snapshot. The first call on this
     * instance must be to this method, so that the register state can be properly sized.
     *
    ...
    /**
     * Ends a local at the given address, using the disposition {@code END_SIMPLY}.
     *
     * @param address {@code >= 0;} the address
    ...
    /**
     * Helper for {@link #endLocal}, which handles the cases where and end local is issued at the
     * same address as a start local for the same register. If this case is found, then this method
     * will remove the start (as the local was never actually active), update the {@link
     * #endIndices} to be accurate, and if needed update the newly-active end to reflect an altered
     * disposition.
     *
     * @param address {@code >= 0;} the address
     * @param endedLocal {@code non-null;} spec representing the local being ended
     * @return {@code true} iff this method found the case in question and adjusted things
     *     accordingly
     */
    ...
    /**
     * Adds or updates an end local (changing its disposition). If this would cause an empty range
     * for a local, this instead removes the local entirely.
     *
     * @param address {@code >= 0;} the address
     * @param disposition {@code non-null;} the disposition
     * @param spec {@code non-null;} spec representing the local
