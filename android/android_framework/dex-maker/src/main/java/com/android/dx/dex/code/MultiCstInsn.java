...
/**
 * An instruction with multiple constant arguments in addition to all the normal instruction
 * information.
 */
public final class MultiCstInsn extends FixedSizeInsn {
  ...
  /**
   * {@code >= NOT_SET;} the constant pool indicies for {@link #constants}, or {@code NOT_SET} if
   * not yet set
   */
  private final int[] index;

  /**
   * {@code >= NOT_SET;} the constant pool index for the class reference in {@link #constants} if
   * any, or {@code NOT_SET} if not yet set
   */
   ...
   */
  public MultiCstInsn(
      Dop opcode, SourcePosition position, RegisterSpecList registers, Constant[] constants) {
  ...
  public DalvInsn withOpcode(Dop opcode) {
    return new MultiCstInsn(
        opcode, getPosition(), getRegisters(), this.constants, this.index, this.classIndex);
  }
  ...
  public DalvInsn withRegisters(RegisterSpecList registers) {
    return new MultiCstInsn(
        getOpcode(), getPosition(), registers, this.constants, this.index, this.classIndex);
  }
  ...
  /**
   * Gets the DEX index of a constant. It is only valid to call this after {@link #setIndex} has
   * been called.
   *
  ...
  public int getIndex(int position) {
    if (!hasIndex(position)) {
      throw new IllegalStateException(
          "index not yet set for constant " + position + " value = " + constants[position]);
    }
  ...
  /**
   * Sets the DEX index of a constant. It is only valid to call this method once per instance.
   *
  ...
  /**
   * Gets the class index associated with this instance. The class index may be set only once and
   * for one of the constants associated with this instance (e.g. a CstMethodRef). It is only valid
   * to call this after {@link #setClassIndex} has been called.
   *
  ...
  /**
   * Sets the class index associated with this instruction. This is the constant pool index for the
   * class referred to by this instruction. Only reference constants have a class, so it is only on
   * instances with reference constants that this method should ever be called. It is only valid to
