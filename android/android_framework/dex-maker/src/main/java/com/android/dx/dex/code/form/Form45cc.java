...
public final class Form45cc extends InsnFormat {
  ...
  /**
   * Gets the number of words required for the given register list, where category-2 values count as
   * two words. Return {@code -1} if the list requires more than five words or contains registers
   * that need more than a nibble to identify them.
   *
   * @param regs {@code non-null;} the register list in question
   * @return {@code >= -1;} the number of words required, or {@code -1} if the list couldn't
   *     possibly fit in this format
   */
  ...
  /**
   * Returns a register list which is equivalent to the given one, except that it splits category-2
   * registers into two explicit entries. This returns the original list if no modification is
   * required
   *
  ...
  public void writeTo(AnnotatedOutput out, DalvInsn insn) {
    ...
    write(
        out, opcodeUnit(insn, makeByte(regG, regA)), regB, codeUnit(regC, regD, regE, regF), regH);
  }
}
