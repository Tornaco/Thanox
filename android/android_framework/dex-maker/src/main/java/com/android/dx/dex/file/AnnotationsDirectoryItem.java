...
public final class AnnotationsDirectoryItem extends OffsettedItem {
  ...
  /**
   * Returns whether this item is a candidate for interning. The only interning candidates are ones
   * that <i>only</i> have a non-null set of class annotations, with no other lists.
   *
   * @return {@code true} if this is an interning candidate, or {@code false} if not
   */
   ...
   * {@inheritDoc}
   *
   * <p><b>Note:</b>: This throws an exception if this item is not internable.
   *
   * @see #isInternable
   */
  @Override
  ...
  /**
   * Sets the direct annotations on this instance. These are annotations made on the class, per se,
   * as opposed to on one of its members. It is only valid to call this method at most once per
   * instance.
   *
  ...
  public void addFieldAnnotations(CstFieldRef field, Annotations annotations, DexFile dexFile) {
    ...
    fieldAnnotations.add(
        new FieldAnnotationStruct(field, new AnnotationSetItem(annotations, dexFile)));
  }
  ...
  public void addMethodAnnotations(CstMethodRef method, Annotations annotations, DexFile dexFile) {
    ...
    methodAnnotations.add(
        new MethodAnnotationStruct(method, new AnnotationSetItem(annotations, dexFile)));
  }
