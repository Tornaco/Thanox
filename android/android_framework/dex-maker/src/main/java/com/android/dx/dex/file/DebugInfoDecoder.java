...
/**
 * A decoder for the dex debug info state machine format. This code exists mostly as a reference
 * implementation and test for for the {@code DebugInfoEncoder}
 */
public class DebugInfoDecoder {
  ...
  /**
   * Gets the decoded locals list, in ascending start-address order. Valid after calling {@code
   * decode}.
   *
  ...
  /**
   * Reads a string index. String indicies are offset by 1, and a 0 value in the stream (-1 as
   * returned by this method) means "null"
   *
        ...
        case DBG_RESTART_LOCAL:
          {
            ...
            try {
              ...
              if (prevle.isStart) {
                throw new RuntimeException(
                    "nonsensical " + "RESTART_LOCAL on live register v" + reg);
              }
  ...
  private static class LocalEntry {
    ...
    public LocalEntry(
        int address, boolean isStart, int reg, int nameIndex, int typeIndex, int signatureIndex) {
      this.address = address;
    ...
    public String toString() {
      return String.format(
          "[%x %s v%d %04x %04x %04x]",
          address, isStart ? "start" : "end", reg, nameIndex, typeIndex, signatureIndex);
    }
  }
}
