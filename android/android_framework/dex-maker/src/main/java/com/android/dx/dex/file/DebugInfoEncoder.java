...
/**
 * An encoder for the dex debug info state machine format. The format for each method enrty is as
 * follows:
 *
 * <ol>
 *   <li>signed LEB128: initial value for line register.
 *   <li>n instances of signed LEB128: string indicies (offset by 1) for each method argument in
 *       left-to-right order with {@code this} excluded. A value of '0' indicates "no name"
 *   <li>A sequence of special or normal opcodes as defined in {@code DebugInfoConstants}.
...
public final class DebugInfoEncoder {
  ...
  private int line = 1;
  /** if non-null: the output to write annotations to. No normal output is written to this. */
   ...
   * @param file {@code null-ok;} may only be {@code null} if simply using this class to do a debug
   *     print
   * @param codeSize
   * @param regSize
   * @param isStatic
  ...
  /**
   * Computes a special opcode that will encode the given position change. If the return value is >
   * 0xff, then the request cannot be fulfilled. Essentially the same as described in "DWARF
   * Debugging Format Version 3" section 6.2.5.1.
   *
   * @param deltaLines {@code >= DBG_LINE_BASE, <= DBG_LINE_BASE + DBG_LINE_RANGE;} the line change
   *     to encode
   * @param deltaAddress {@code >= 0;} the address change to encode
   * @return {@code <= 0xff} if in range, otherwise parameters are out of range
   */
  ...
  /**
   * Annotates or writes a message to the {@code debugPrint} writer if applicable.
   *
   ...
   *
   * @return {@code non-null;} encoded byte sequence without padding and terminated with a {@code
   *     0x00} byte
   */
  public byte[] convert() {
    try {
  ...
  /**
   * Converts and produces annotations on a stream. Does not write actual bits to the {@code
   * AnnotatedOutput}.
   *
   * @param prefix {@code null-ok;} prefix to attach to each line of output
   * @param debugPrint {@code null-ok;} if specified, an alternate output for annotations
   ...
   */
  public byte[] convertAndAnnotate(
      String prefix, PrintWriter debugPrint, AnnotatedOutput out, boolean consume) {
    this.prefix = prefix;
  ...
  /**
   * Emits all local variable activity that occurs at the current {@link #address} starting at the
   * given index into {@code locals} and including all subsequent activity at the same address.
   *
   ...
   * @throws IOException
   */
  private int emitPositionsAtAddress(
      int curPositionIdx, ArrayList<PositionList.Entry> sortedPositions) throws IOException {
    int positionsSz = sortedPositions.size();
  ...
  /**
   * Emits the header sequence, which consists of LEB128-encoded initial line number and string
   * indicies for names of all non-"this" arguments.
   *
   * @param sortedPositions positions, sorted by ascending address
   * @param methodArgs local list entries for method argumens arguments, in left-to-right order
   *     omitting "this"
   * @throws IOException
   */
  private void emitHeader(
      ArrayList<PositionList.Entry> sortedPositions, ArrayList<LocalList.Entry> methodArgs)
      throws IOException {
  ...
  private ArrayList<PositionList.Entry> buildSortedPositions() {
    ...
    Collections.sort(
        result,
        new Comparator<PositionList.Entry>() {
          @Override
  ...
  /**
   * Gets the register that begins the method's parameter range (including the 'this' parameter for
   * non-static methods). The range continues until {@code regSize}
   *
  ...
  /**
   * Extracts method arguments from a locals list. These will be collected from the input list and
   * sorted by ascending register in the returned list.
   *
   * @return list of non-{@code this} method argument locals, sorted by ascending register
   */
  private ArrayList<LocalList.Entry> extractMethodArguments() {
    ...
    Collections.sort(
        result,
        new Comparator<LocalList.Entry>() {
          @Override
  ...
  /**
   * Returns a string representation of this LocalList entry that is appropriate for emitting as an
   * annotation.
   *
  ...
  /**
   * Emits a {@link DebugInfoConstants#DBG_RESTART_LOCAL DBG_RESTART_LOCAL} sequence.
   *
  ...
  /**
   * Emits a string index as an unsigned LEB128. The actual value written is shifted by 1, so that
   * the '0' value is reserved for "null". The null symbol is used in some cases by the parameter
   * name list at the beginning of the sequence.
   *
  ...
  /**
   * Emits a type index as an unsigned LEB128. The actual value written is shifted by 1, so that the
   * '0' value is reserved for "null".
   *
  ...
  /**
   * Emits a {@link DebugInfoConstants#DBG_START_LOCAL DBG_START_LOCAL} or {@link
   * DebugInfoConstants#DBG_START_LOCAL_EXTENDED DBG_START_LOCAL_EXTENDED} sequence.
   *
  ...
  /**
   * Emits a {@link DebugInfoConstants#DBG_START_LOCAL_EXTENDED DBG_START_LOCAL_EXTENDED} sequence.
   *
   * @param entry entry to emit
   * @throws IOException
   */
  ...
  /**
   * Emits the necessary byte sequences to emit the given position table entry. This will typically
   * be a single special opcode, although it may also require DBG_ADVANCE_PC or DBG_ADVANCE_LINE.
   *
  ...
  /**
   * Emits an {@link DebugInfoConstants#DBG_ADVANCE_LINE DBG_ADVANCE_LINE} sequence.
   *
  ...
  /**
   * Emits an {@link DebugInfoConstants#DBG_ADVANCE_PC DBG_ADVANCE_PC} sequence.
   *
   ...
   *
   * @param n {@code >= 0;} value to emit. Note that, although this can represent integers larger
   *     than Integer.MAX_VALUE, we currently don't allow that.
   * @throws IOException
   */
  ...
  /** Emits the {@link DebugInfoConstants#DBG_END_SEQUENCE DBG_END_SEQUENCE} bytecode. */
