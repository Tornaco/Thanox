...
/**
 * A section of a {@code .dex} file which consists of a sequence of {@link Item} objects. Each of
 * the items must have the same size in the output.
 */
public abstract class UniformItemSection extends Section {
  ...
  /**
   * Gets the item corresponding to the given {@link Constant}. This will throw an exception if the
   * constant is not found, including if this instance isn't the sort that maps constants to {@link
  ...
  /**
   * Alters or picks the order for items in this instance if desired, so that subsequent calls to
   * {@link #items} will yield a so-ordered collection. If the items in this instance are indexed,
