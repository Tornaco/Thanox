...
/**
 * Class that represents a contiguous list of uniform items. Each item in the list, in particular,
 * must have the same write size and alignment.
 *
 * <p>This class inherits its alignment from its items, bumped up to {@code 4} if the items have a
 * looser alignment requirement. If it is more than {@code 4}, then there will be a gap after the
...
public final class UniformListItem<T extends OffsettedItem> extends OffsettedItem {
  ...
  /**
   * Constructs an instance. It is illegal to modify the given list once it is used to construct an
  ...
  /**
   * Helper for {@link #UniformListItem}, which returns the alignment requirement implied by the
   * given list. See the header comment for more details.
   *
