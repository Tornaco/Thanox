...
public final class DexMerger {
  ...
  private DexMerger(
      Dex[] dexes, CollisionPolicy collisionPolicy, DxContext context, WriterSizes writerSizes)
      throws IOException {
    this.dexes = dexes;
  ...
  public Dex merge() throws IOException {
    ...
    if (wastedByteCount > +compactWasteThreshold) {
      ...
      context.out.printf(
          "Result compacted from %.1fKiB to %.1fKiB to save %.1fKiB%n",
    ...
    for (int i = 0; i < dexes.length; i++) {
      context.out.printf(
          "Merged dex #%d (%d defs/%.1fKiB)%n",
          i + 1, dexes[i].getTableOfContents().classDefs.size, dexes[i].getLength() / 1024f);
    }
    context.out.printf(
  ...
  /**
   * Returns the union of classes from both files, sorted in order such that a class is always
  ...
  /** Reads just enough data on each class so that we can sort it and then find it later. */
   ...
   *
   * <p>TODO: this may write multiple copies of the same annotation set. We should shrink the output
  ...
  /** Reads a class_def_item beginning at {@code in} and writes the index and data. */
  ...
  /**
   * Byte counts for the sections written when creating a dex. Target sizes are defined in one of
   * two ways:
   *
   * <ul>
   *   <li>By pessimistically guessing how large the union of dex files will be. We're pessimistic
   *       because we can't predict the amount of duplication between dex files, nor can we predict
   *       the length of ULEB-encoded offsets or indices.
  ...
  private static class WriterSizes {
    ...
    private void plus(TableOfContents contents, boolean exact) {
      idsDefs +=
          contents.stringIds.size * SizeOf.STRING_ID_ITEM
    ...
    public int size() {
      return header
          ...
          + annotationsSet
          + annotationsSetRefList
          + annotation;
    }
  }

  /**
   * Reads an IDs section of two dex files and writes an IDs section of a merged dex file. Populates
   * maps from old to new indices in the process.
   */
  abstract class IdMerger<T extends Comparable<T>> {
    ...
    public final void mergeSorted() {
      ...
      while (!values.isEmpty()) {
        ...
        for (Integer dex : first.getValue()) {
          ...
          offsets[dex] =
              readIntoMap(
                  dexSections[dex], sections[dex], indexMaps[dex], indexes[dex], values, dex);
        }
        write(first.getKey());
        outCount++;
      }
    ...
    private int readIntoMap(
        Dex.Section in,
        TableOfContents.Section section,
        IndexMap indexMap,
        int index,
        TreeMap<T, List<Integer>> values,
        int dex) {
    ...
    /** Merges unsorted sections by reading them completely into memory and sorting in memory. */
