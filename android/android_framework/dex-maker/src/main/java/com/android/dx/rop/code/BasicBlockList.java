...
public final class BasicBlockList extends LabeledList {
  /**
   * {@code >= -1;} the count of registers required by this method or {@code -1} if not yet
   * calculated
   */
  private int regCount;

  /**
   * Constructs an instance. All indices initially contain {@code null}, and the first-block label
   * is initially {@code -1}.
   *
  ...
  /**
   * Returns how many registers this method requires. This is simply the maximum of
   * register-number-plus-category referred to by this instance's instructions (indirectly through
   * {@link BasicBlock} instances).
   *
  ...
  /**
   * Gets the total instruction count for this instance. This is the sum of the instruction counts
   * of each block.
   *
  ...
  /**
   * Gets the total instruction count for this instance, ignoring mark-local instructions which are
   ...
   * instruction are offset by the given amount. Mutability of the result is inherited from the
   * original.
   *
  ...
  /**
   * Gets the preferred successor for the given block. If the block only has one successor, then
   * that is the preferred successor. Otherwise, if the block has a primay successor, then that is
   * the preferred successor. If the block has no successors, then this returns {@code null}.
   *
  ...
  /**
   * Compares the catches of two blocks for equality. This includes both the catch types and target
   * labels.
   *
   * @param block1 {@code non-null;} one block to compare
   * @param block2 {@code non-null;} the other block to compare
   * @return {@code true} if the two blocks' non-primary successors are identical
   */
