...
public class InvokePolymorphicInsn extends Insn {
  ...
  /**
   * {@code non-null;} method as it appears at the call site of the original invoke-virtual
   * instruction. This is used to construct the invoke method to target and the call-site prototype.
   */
   ...
   * @param callSiteMethod {@code non-null;} the method called by invoke-virtual that this instance
   *     will replace.
   */
  public InvokePolymorphicInsn(
      Rop opcode,
    ...
    if (opcode.getBranchingness() != Rop.BRANCH_THROW) {
      throw new IllegalArgumentException(
          "opcode with invalid branchingness: " + opcode.getBranchingness());
    }
  ...
  private static CstMethodRef makePolymorphicMethod(final CstMethodRef callSiteMethod) {
    ...
    throw new IllegalArgumentException(
        "Unknown signature polymorphic method: " + callSiteMethod.toHuman());
  }
  ...
  public Insn withAddedCatch(Type type) {
    return new InvokePolymorphicInsn(
        getOpcode(), getPosition(), getSources(), catches.withAddedType(type), getCallSiteMethod());
  }
  ...
  public Insn withRegisterOffset(int delta) {
    return new InvokePolymorphicInsn(
        getOpcode(), getPosition(), getSources().withOffset(delta), catches, getCallSiteMethod());
  }
  ...
  public Insn withNewRegisters(RegisterSpec result, RegisterSpecList sources) {
    return new InvokePolymorphicInsn(
        getOpcode(), getPosition(), sources, catches, getCallSiteMethod());
  }
  ...
  /**
   * Gets the method to be invoked. This will be will either be {@code
   * java.lang.invoke.MethodHandle.invoke()} or {@code java.lang.invoke.MethodHandle.invokeExact()}.
   *
  ...
  /**
   * Gets the call site prototype. The call site prototype is provided as an argument to
   * invoke-polymorphic to enable type checking and type conversion.
   *
