...
public final class LocalVariableInfo extends MutabilityControl {
  ...
  /**
   * {@code non-null;} {@link RegisterSpecSet} to use when indicating a block that has no locals; it
   * is empty and immutable but has an appropriate max size for the method
   */
  private final RegisterSpecSet emptySet;

  /**
   * {@code non-null;} array consisting of register sets representing the sets of variables already
   * assigned upon entry to each block, where array indices correspond to block labels
   */
  ...
  /**
   * Gets the register set associated with the start of the block with the given label. This returns
   ...
   * have a simple type and the one in the instruction can be an arbitrary {@link TypeBearer} (such
   * as a constant value).
   *
