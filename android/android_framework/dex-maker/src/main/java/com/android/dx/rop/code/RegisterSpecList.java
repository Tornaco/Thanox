...
public final class RegisterSpecList extends FixedSizeList implements TypeList {
   ...
   * @return {@code non-null;} an appropriately-constructed instance
   */
  public static RegisterSpecList make(
      RegisterSpec spec0, RegisterSpec spec1, RegisterSpec spec2, RegisterSpec spec3) {
  ...
  /**
   * Returns a RegisterSpec in this list that uses the specified register, or null if there is none
   * in this list.
   *
  ...
  /**
   * Returns the index of a RegisterSpec in this list that uses the specified register, or -1 if
   * none in this list uses the register.
   *
   * @param reg Register to find
  ...
  /**
   * Gets the minimum required register count implied by this instance. This is equal to the highest
   * register number referred to plus the widest width (largest category) of the type used in that
   * register.
   *
  ...
  /**
   * Returns a new instance, which is the same as this instance, except that it has an additional
   * element prepended to the original. Mutability of the result is inherited from the original.
   *
  ...
  /**
   * Returns a new instance, which is the same as this instance, except that its first element is
   * removed. Mutability of the result is inherited from the original.
   *
   * @return {@code non-null;} an appropriately-constructed instance
   */
  ...
  /**
   * Returns a new instance, which is the same as this instance, except that its last element is
   * removed. Mutability of the result is inherited from the original.
   *
   * @return {@code non-null;} an appropriately-constructed instance
   */
   ...
   * Indexes in the BitSet with a zero are included, while indexes with a one are excluded.
  ...
  /**
   * Returns an instance that is identical to this one, except that all incompatible register
   * numbers are renumbered sequentially from the given base, with the first number duplicated if
   * indicated. If a null BitSet is given, it indicates all registers are incompatible.
   *
   * @param base the base register number
   * @param duplicateFirst whether to duplicate the first number
   * @param compatRegs {@code null-ok;} either a {@code non-null} set of compatible registers, or
   *     {@code null} to indicate all registers are incompatible
   * @return {@code non-null;} an appropriately-constructed instance
   */
  public RegisterSpecList withExpandedRegisters(
      int base, boolean duplicateFirst, BitSet compatRegs) {
    int sz = size();
  ...
  private static class Expander {
    ...
    private Expander(
        RegisterSpecList regSpecList, BitSet compatRegs, int base, boolean duplicateFirst) {
