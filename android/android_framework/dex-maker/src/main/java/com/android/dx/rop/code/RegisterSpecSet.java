...
/**
 * Set of {@link RegisterSpec} instances, where a given register number may appear only once in the
 * set.
 */
public final class RegisterSpecSet extends MutabilityControl {
  ...
  /**
   * {@code non-null;} array of register specs, where each element is {@code null} or is an instance
   * whose {@code reg} matches the array index
   */
   ...
   *
   * @param maxSize {@code >= 0;} the maximum register number (exclusive) that may be represented in
   *     this instance
   */
  ...
  /**
   * Gets the maximum number of registers that may be in this instance, which is also the
   * maximum-plus-one of register numbers that may be represented.
   *
   ...
   * @return {@code null-ok;} the element with the given register number or {@code null} if there is
   *     none
   */
  ...
  /**
   * Gets the element with the same register number as the given spec, if any. This is just a
   * convenient shorthand for {@code get(spec.getReg())}.
   *
   * @param spec {@code non-null;} spec with the desired register number
   * @return {@code null-ok;} the element with the matching register number or {@code null} if there
   *     is none
   */
  ...
  /**
   * Returns the spec in this set that's currently associated with a given local (type, name, and
   * signature), or {@code null} if there is none. This ignores the register number of the given
   * spec but matches on everything else.
   *
  ...
  /**
   * Returns the spec in this set that's currently associated with a given local (name and
   * signature), or {@code null} if there is none.
   *
  ...
  /**
   * Puts the given spec into the set. If there is already an element in the set with the same
   * register number, it is replaced. Additionally, if the previous element is for a category-2
   * register, then that previous element is nullified. Finally, if the given spec is for a
   * category-2 register, then the immediately subsequent element is nullified.
   *
  ...
  /**
   * Intersects this instance with the given one, modifying this instance. The intersection consists
   * of the pairwise {@link RegisterSpec#intersect} of corresponding elements from this instance and
   * the given one where both are non-null.
   *
   ...
   *     then the only non-null result elements occur when registers being intersected have equal
   *     names (or both have {@code null} names)
   */
  ...
  /**
   * Returns an instance that is identical to this one, except that all register numbers are offset
   * by the given amount. Mutability of the result is inherited from the original.
   *
