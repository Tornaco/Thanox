...
/**
 * Representation of a method descriptor. Instances of this class are generally interned and may be
 * usefully compared with each other using {@code ==}.
 */
public final class Prototype implements Comparable<Prototype> {
  ...
  /**
   * Constructs an instance. This is a private constructor; use one of the public static methods to
   * get instances.
   *
  ...
  /**
   * Returns the unique instance corresponding to the given method descriptor. See vmspec-2 sec4.3.3
   ...
   *
   * <p>The {@code Prototype} returned will be the interned value if present, or a new instance
   * otherwise. If a new instance is created, it is not placed in the intern table.
   *
   * @param descriptor {@code non-null;} the descriptor
   * @return {@code non-null;} the corresponding instance
   * @throws IllegalArgumentException thrown if the descriptor has invalid syntax
  ...
  /**
   * Helper for {@link #intern} which returns an empty array to populate with parsed parameter
   * types, and which also ensures that there is a '(' at the start of the descriptor and a single
   * ')' somewhere before the end.
   *
   * @param descriptor {@code non-null;} the descriptor string
   * @return {@code non-null;} array large enough to hold all parsed parameter types, but which is
   *     likely actually larger than needed
   */
  ...
  /**
   * Interns an instance, adding to the descriptor as necessary based on the given definer, name,
   * and flags. For example, an init method has an uninitialized object of type {@code definer} as
   * its first argument.
   *
   * @param descriptor {@code non-null;} the descriptor string
   ...
   */
  public static Prototype intern(
      String descriptor, Type definer, boolean isStatic, boolean isInit) {
  ...
  /**
   * Interns an instance which consists of the given number of {@code int}s along with the given
   * return type
   *
  ...
  /**
   * Gets the list of frame types corresponding to the list of parameter types. The difference
   * between the two lists (if any) is that all "intlike" types (see {@link Type#isIntlike}) are
   * replaced by {@link Type#INT}.
   *
  ...
  /**
   * Returns a new interned instance, which is the same as this instance, except that it has an
   * additional parameter prepended to the original's argument list.
   *
