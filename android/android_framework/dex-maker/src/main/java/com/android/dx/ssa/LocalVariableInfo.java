...
/**
 * Container for local variable information for a particular {@link SsaMethod}.
...
public class LocalVariableInfo extends MutabilityControl {
  ...
  /**
   * {@code non-null;} {@link RegisterSpecSet} to use when indicating a
  ...
  /**
   * {@code non-null;} array consisting of register sets representing the sets of variables already
   * assigned upon entry to each block, where array indices correspond to block indices
   */
  ...
  /**
   * Merges the given register set into the set for the block with the given index. If there was not
   ...
   *
   * @param index {@code >= 0;} the block index
   * @param specs {@code non-null;} the register set to merge into the start set for the block
   * @return {@code true} if the merge resulted in an actual change to the associated set (including
   *     storing one for the first time) or {@code false} if there was no change
   */
  ...
  /**
   * Gets the register set associated with the start of the block with the given index. This returns
   * an empty set with the appropriate max size if no set was associated with the block in question.
   *
   * @param index {@code >= 0;} the block index
   ...
   * shorthand for {@code getStarts(block.getLabel())}.
   *
   ...
   * index. This returns a newly-allocated empty {@link RegisterSpecSet} of appropriate max size if
  ...
  /**
   * Adds an assignment association for the given instruction and register spec. This throws an
   * exception if the instruction doesn't actually perform a named variable assignment.
   *
   * <p><b>Note:</b> Although the instruction contains its own spec for the result, it still needs
   * to be passed in explicitly to this method, since the spec that is stored here should always
   * have a simple type and the one in the instruction can be an arbitrary {@link
   * com.android.dx.rop.type.TypeBearer} (such as a constant value).
   *
  ...
  /**
   * Gets the named register being assigned by the given instruction, if previously stored in this
   * instance.
   *
   ...
   *
   * @param index {@code >= 0;} the block index
   * @return {@code null-ok;} associated register set or {@code null} if there is none
   */
