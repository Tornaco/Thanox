...
/**
 * Resolves the result types of phi instructions. When phi instructions are inserted, their result
 * types are set to BT_VOID (which is a nonsensical type for a register) but must be resolve to a
 * real type before converting out of SSA form.
 *
 * <p>
 *
 * <p>The resolve is done as an iterative merge of each phi's operand types. Phi operands may be
 * themselves be the result of unresolved phis, and the algorithm tries to find the most-fit type
 * (for example, if every operand is the same constant value or the same local variable info, we
 * want that to be reflected).
 *
 * <p>
 *
 * <p>This algorithm assumes a dead-code remover has already removed all circular-only phis that may
 * have been inserted.
 */
public class PhiTypeResolver {
  ...
  /**
   * Resolves the result of a phi insn based on its operands. The "void" type, which is a
   * nonsensical type for a register, is used for registers defined by as-of-yet-unresolved phi
   * operations.
   *
