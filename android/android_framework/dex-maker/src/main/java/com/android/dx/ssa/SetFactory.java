...
public final class SetFactory {
   ...
   *
   * <p>A value of 3072 here is somewhere around 1.125mb of total bitset size.
   */
  private static final int DOMFRONT_SET_THRESHOLD_SIZE = 3072;

  /**
   * BitIntSet/ListIntSet threshold for interference graph sets. These sets are kept per register
   * until register allocation is done.
   *
   * <p>A value of 3072 here is somewhere around 1.125mb of total bitset size.
   */
  ...
  /**
   * BitIntSet/ListIntSet threshold for the live in/out sets kept by {@link SsaBasicBlock}. These
   * are sets of SSA registers kept per basic block during register allocation.
   *
   * <p>The total size of a bitset for this would be the count of blocks times the size of
   * registers. The threshold value here is merely the register count, which is typically on the
   * order of the block count as well.
   */
  ...
  /**
   * Make IntSet for the interference graph sets. Public because InterferenceGraph is in another
   * package.
   *
