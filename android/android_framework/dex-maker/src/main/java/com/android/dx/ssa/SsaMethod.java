...
public final class SsaMethod {
  ...
  /**
   * indexed by register: the insn where said register is defined or null if undefined. null until
   * (lazily) created.
   */
  ...
  /**
   * "back-convert mode". Set during back-conversion when registers are about to be mapped into a
   * non-SSA namespace. When true, use and def lists are unavailable.
   *
   * <p>TODO: Remove this mode, and place the functionality elsewhere
   */
   ...
   *
   * @param ropMethod {@code non-null;} the original rop-form method that this instance is based on
   * @param paramWidth the total width, in register-units, of the method's parameters
   * @param isStatic {@code true} if this method has no {@code this} pointer argument
   */
  ...
  /**
   * Builds a BitSet of block indices from a basic block list and a list of labels taken from Rop
   * form.
   *
  ...
  /**
   * Builds an IntList of block indices from a basic block list and a list of labels taken from Rop
   * form.
   *
   ...
   *
   * @param block block to which this GOTO will be added (not it's destination!)
  ...
  /**
   * Creates an exit block and attaches it to the CFG if this method exits. Methods that never exit
   * will not have an exit block. This is called after edge-splitting and phi insertion, since the
   * edges going into the exit block should not be considered in those steps.
   */
  ...
  /**
   * Makes a new basic block for this method, which is empty besides a single {@code GOTO}.
   * Successors and predecessors are not yet set.
   *
   * @return new block
   */
  ...
  /** @return {@code null-ok;} block of exit block or {@code null} if there is none */
  ...
  /**
   * Borrows a register to use as a temp. Used in the phi removal process. Call
   ...
   * @return insn (actual instance from code) that defined this reg or null if reg is not defined.
   */
   ...
   *
   * @param insn {@code insn non-null;} insn being changed. {@code insn.getSources()} must return
   *     the new source list.
  ...
  /**
   * Removes a given {@code insn} from the use lists for the given {@code oldSources} (rather than
   * the sources currently returned by insn.getSources()).
   *
   * @param insn {@code non-null;} insn in question
   * @param oldSources {@code null-ok;} registers whose use lists {@code insn} should be removed
   *     form
   */
  ...
  /**
   * Indicates that the instruction list has changed or the SSA register count has increased, so
   * that internal datastructures that rely on it should be rebuild. In general, the various other
   * on* methods should be called in preference when changes occur if they are applicable.
   */
   ...
   *
   * @param insn {@code non-null;} insn who's result should be recorded as a definition
   * @param oldResult {@code null-ok;} a previous result that should be no longer considered a
   *     definition by this insn
   */
  ...
  /**
   * Checks to see if the given SSA reg is ever associated with a local local variable. Each SSA reg
   * may be associated with at most one local var.
   *
  ...
  /**
   * Walks the basic block tree in depth-first order, calling the visitor method once for every
   * block. This depth-first walk may be run forward from the method entry point or backwards from
   * the method exit points.
   *
   * @param reverse true if this should walk backwards from the exit points
   * @param v {@code non-null;} callback interface. {@code parent} is set unless this is the root
   *     node
   */
  ...
  /**
   * Visits blocks in dom-tree order, starting at the current node. The {@code parent} parameter of
   * the Visitor.visitBlock callback is currently always set to null.
   *
  ...
  /**
   * Sets "back-convert mode". Set during back-conversion when registers are about to be mapped into
   * a non-SSA namespace. When true, use and def lists are unavailable.
   */
