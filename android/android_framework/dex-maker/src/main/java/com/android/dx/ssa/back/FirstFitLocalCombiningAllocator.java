...
/**
 * Allocates registers in a first-fit fashion, with the bottom reserved for method parameters and
 * all SSAregisters representing the same local variable kept together if possible.
 */
public class FirstFitLocalCombiningAllocator extends RegisterAllocator {
   ...
   * @param minimizeRegisters true if converter should take steps to minimize rop-form registers
   */
  public FirstFitLocalCombiningAllocator(
      SsaMethod ssaMeth, InterferenceGraph interference, boolean minimizeRegisters) {
  ...
  /**
   * Gets the parameter index for SSA registers that are method parameters. {@code -1} is returned
   * for non-parameter registers.
   *
  ...
  /**
   * Maps all local-associated registers that are not parameters. Tries to find an unreserved range
   * that's wide enough for all of the SSA registers, and then tries to map them all to that range.
   * If not all fit, a new range is tried until all registers have been fit.
   */
   ...
   * SSA registers that don't fit are left unmapped.
   *
   * @param specs {@code non-null;} SSA registers to attempt to map
   * @param ropReg {@code >=0;} rop register to map to
   * @param maxAllowedCategory {@code 1..2;} maximum category allowed in mapping.
   * @param markReserved do so if {@code true}
   * @return {@code true} if all registers were mapped, {@code false} if some remain unmapped
   */
  private boolean tryMapRegs(
      ArrayList<RegisterSpec> specs, int ropReg, int maxAllowedCategory, boolean markReserved) {
   ...
   * @param maxAllowedCategory {@code 1..2;} the maximum category that the SSA register is allowed
   *     to be
  ...
  /**
   * Checks to see if any rop registers in the specified range are reserved for local variables or
   * parameters.
   *
  ...
  /**
   * Finds rop registers that can be used for local variables. If {@code MIX_LOCALS_AND_OTHER} is
   * {@code false}, this means any rop register that has not yet been used.
   *
   * @param startReg {@code >= 0;} a rop register to start the search at
  ...
  /**
   * Maps any parameter that isn't local-associated, which can happen in the case where there is no
   * java debug info.
   */
  ...
  /**
   * Handles check cast results to reuse the same source register. Inserts a move if it can't map
   * the same register to both and the check cast is not caught.
   */
  private void handleCheckCastResults() {
    for (NormalSsaInsn insn : moveResultPseudoInsns) {
      ...
      /** If the predecessor block has a check-cast, it will be the last instruction */
      ...
      /**
       * See if either register is already mapped. Most likely the move result will be mapped
       * already since the cast result is stored in a local variable.
       */
      ...
      if (moveRopReg != checkRopReg && !hasExceptionHandlers) {
        ((NormalSsaInsn) checkCastInsn)
            .changeOneSource(0, insertMoveBefore(checkCastInsn, checkRegSpec));
  ...
  /**
   * Checks to see if a list of SSA registers can all be mapped into the same rop reg. Ignores
   * registers that have already been mapped, and checks the interference graph and ensures the
   * range does not cross the parameter range.
   *
  ...
  /**
   * Checks to see if {@code ssaSpec} can be mapped to {@code ropReg}. Checks interference graph and
   * ensures the range does not cross the parameter range.
   *
  ...
  /**
   * Returns true if the specified rop register + category will cross the boundry between the lower
   * {@code paramWidth} registers reserved for method params and the upper registers. We cannot
   * allocate a register that spans the param block and the normal block, because we will be moving
   * the param block to high registers later.
   *
   ...
   * move-result-pseudo/invoke-range instrucitons.
   */
  private void analyzeInstructions() {
    ssaMeth.forEachInsn(
        new SsaInsn.Visitor() {
           ...
           *
           * <p>1) Adds a local variable assignment to the {@code localVariables} map. 2) Add
           * move-result-pseudo to the {@code moveResultPseudoInsns} list. 3) Add invoke-range to
           * the {@code invokeRangeInsns} list.
           *
           * @param insn {@code non-null;} insn that may represent a local variable assignment
           */
  ...
  /**
   * Adds a mapping from an SSA register to a rop register. {@link #canMapReg} should have already
   * been called.
   *
  ...
  /**
   * Maps the source registers of the specified instruction such that they will fall in a contiguous
   * range in rop form. Moves are inserted as necessary to allow the range to be allocated.
   *
  ...
  /**
   * Find a contiguous rop register range that fits the specified instruction's sources. First, try
   * to center the range around sources that have already been mapped to rop registers. If that
   * fails, just find a new contiguous range that doesn't interfere.
   *
   * @param insn {@code non-null;} the insn whose sources need to fit. Must be last insn in basic
   *     block.
  ...
  private int findRangeAndAdjust(NormalSsaInsn insn) {
    ...
    for (int i = resultMovesRequired.nextSetBit(0);
        i >= 0;
  ...
  /**
   * Finds an unreserved range that will fit the sources of the specified instruction. Does not
   * bother trying to center the range around an already-mapped source register;
   *
   * @param insn {@code non-null;} insn to build range for
   * @param rangeLength {@code >=0;} length required in register units
   * @param categoriesForIndex {@code non-null;} indexed by source index; the category for each
   *     source
   * @param outMovesRequired {@code non-null;} an output parameter indexed by source index that will
   *     contain the set of sources which need moves inserted
   * @return the rop register that starts the fitting range
   */
  private int findAnyFittingRange(
      NormalSsaInsn insn, int rangeLength, int[] categoriesForIndex, BitSet outMovesRequired) {
    Alignment alignment = Alignment.UNSPECIFIED;
   ...
   * @param outMovesRequired {@code non-null;} an output parameter indexed by source index that will
   *     contain the set of sources which need moves inserted
   * @return the width of the fit that that does not involve added moves or {@code -1} if "no fit
   *     possible"
   */
  private int fitPlanForRange(
      int ropReg, NormalSsaInsn insn, int[] categoriesForIndex, BitSet outMovesRequired) {
    RegisterSpecList sources = insn.getSources();
    int szSources = sources.size();
    int fitWidth = 0;
  ...
  /**
   * Converts a bit set of SSA registers into a RegisterSpecList containing the definition specs of
   * all the registers.
   *
   ...
   * mappings first, from most to least common. If none of the registers have mappings yet, a new
   * mapping is created.
   */
  ...
  private static class Multiset {
    ...
    /**
     * Searches the set for the element that has been added the most. In the case of a tie, the
     * element that was added first is returned. Then, it clears the count on that element. The size
