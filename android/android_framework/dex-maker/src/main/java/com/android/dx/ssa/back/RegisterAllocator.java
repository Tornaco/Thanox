...
public abstract class RegisterAllocator {
   ...
   * @param interference Interference graph, indexed by register in both dimensions.
   */
  ...
  /**
   * Indicates whether the method params were allocated at the bottom of the namespace, and thus
   * should be moved up to the top of the namespace after phi removal.
   *
  ...
  /**
   * Returns the category (width) of the definition site of the register. Returns {@code 1} for
   * undefined registers.
   *
   * @param reg register
   * @return {@code 1..2}
   ...
   * @return definition spec of the register or null if it is never defined (for the case of
   *     "version 0" SSA registers)
   */
  ...
  /**
   * Returns true if the definition site of this register is a move-param (ie, this is a method
   * parameter).
   *
  ...
  /**
   * Inserts a move instruction for a specified SSA register before a specified instruction,
   * creating a new SSA register and adjusting the interference graph in the process. The insn
   * currently must be the last insn in a block.
   *
   * @param insn {@code non-null;} insn to insert move before, must be last insn in block
  ...
  protected final RegisterSpec insertMoveBefore(SsaInsn insn, RegisterSpec reg) {
    ...
    SsaInsn toAdd =
        SsaInsn.makeFromRop(
            new PlainInsn(
                Rops.opMove(newRegSpec.getType()),
                SourcePosition.NO_INFO,
                newRegSpec,
                RegisterSpecList.make(reg)),
            block);
