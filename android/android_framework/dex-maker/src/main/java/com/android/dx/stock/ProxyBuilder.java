 ...
 *
 * <p>This is similar to the {@code java.lang.reflect.Proxy} class, but works for classes instead of
 * interfaces.
 *
 * <h3>Example</h3>
 *
 ...
 * <h3>Usage</h3>
 *
 * Call {@link #forClass(Class)} for the Class you wish to proxy. Call {@link
 * #handler(InvocationHandler)} passing in an {@link InvocationHandler}, and then call {@link
 * #build()}. The returned instance will be a dynamically generated subclass where all method calls
 * will be delegated to the invocation handler, except as noted below.
 *
 * <p>The static method {@link #callSuper(Object, Method, Object...)} allows you to access the
 * original super method for a given proxy. This allows the invocation handler to selectively
 * override some methods but not others.
 *
 * <p>By default, the {@link #build()} method will call the no-arg constructor belonging to the
 * class being proxied. If you wish to call a different constructor, you must provide arguments for
 * both {@link #constructorArgTypes(Class[])} and {@link #constructorArgValues(Object[])}.
 *
 * <p>This process works only for classes with public and protected level of visibility.
 *
 * <p>You may proxy abstract classes. You may not proxy final classes.
 *
 * <p>Only non-private, non-final, non-static methods will be dispatched to the invocation handler.
 * Private, static or final methods will always call through to the superclass as normal.
 *
 * <p>The {@link #finalize()} method on {@code Object} will not be proxied.
 *
 * <p>You must provide a dex cache directory via the {@link #dexCache(File)} method. You should take
 * care not to make this a world-writable directory, so that third parties cannot inject code into
 * your application. A suitable parameter for these output directories would be something like this:
 *
 * <pre>{@code
 * getApplicationContext().getDir("dx", Context.MODE_PRIVATE);
 * }</pre>
 *
 * <p>If the base class to be proxied leaks the {@code this} pointer in the constructor (bad
 * practice), that is to say calls a non-private non-final method from the constructor, the
 * invocation handler will not be invoked. As a simple concrete example, when proxying Random we
 * discover that it internally calls setSeed during the constructor. The proxy will not intercept
 * this call during proxy construction, but will intercept as normal afterwards. This behaviour may
...
public final class ProxyBuilder<T> {
   ...
   *
   * <p>This is required for methods whose return type is primitive, since the {@link
   * InvocationHandler} will return us a boxed result, and we'll need to convert it back to
   * primitive value.
   */
   ...
   *
   * <p>If you create a proxy with {@link #build()}, the proxy will already have a handler set,
   * provided that you configured one with {@link #handler(InvocationHandler)}.
   *
   * <p>If you generate a proxy class with {@link #buildProxyClass()}, instances of the proxy class
  ...
  private static <T, G extends T> void generateCodeForAllMethods(
      DexMaker dexMaker,
    ...
    MethodId<InvocationHandler, Object> methodInvoke =
        handlerType.getMethod(TypeId.OBJECT, "invoke", TypeId.OBJECT, methodType, objectArrayType);
    for (int m = 0; m < methodsToProxy.length; ++m) {
       ...
       *     }
       */
      MethodId<G, ?> callsSuperMethod =
          generatedType.getMethod(resultType, superMethodName(method), argTypes);
      ...
      if ((method.getModifiers() & ABSTRACT) == 0) {
        ...
        Local<AbstractMethodError> superAbstractMethodError =
            superCode.newLocal(abstractMethodErrorClass);
        throwAbstractMethodError(
            superCode, method, superAbstractMethodErrorMessage, superAbstractMethodError);
      }
    }
  }
  ...
  private static void invokeSuper(
      MethodId superMethod, Code superCode, Local superThis, Local[] superArgs, Local superResult) {
  ...
  /**
   * The super method must include the return type, otherwise its ambiguous for methods with
  ...
  private static <T, G extends T> void generateConstructorsAndFields(
      DexMaker dexMaker, TypeId<G> generatedType, TypeId<T> superType, Class<T> superClass) {
    TypeId<InvocationHandler> handlerType = TypeId.get(InvocationHandler.class);
    TypeId<Method[]> methodArrayType = TypeId.get(Method[].class);
    FieldId<G, InvocationHandler> handlerField =
        generatedType.getField(handlerType, FIELD_NAME_HANDLER);
   ...
   *
   * <p>A void method will not return anything. A method that returns a primitive will need to unbox
   * the boxed result. Otherwise we will cast the result.
   */
  // This one is tricky to fix, I gave up.
  @SuppressWarnings({"rawtypes", "unchecked"})
  private static void generateCodeForReturnStatement(
      Code code,
      Class methodReturnType,
      Local localForResultOfInvoke,
  ...
  /**
   * Sets the directory where executable code is stored. See {@link DexMaker#generateAndLoad
   * DexMaker.generateAndLoad()} for guidance on choosing a secure location for the dex cache.
   */
   ...
   *
   * @throws UnsupportedOperationException if the class we are trying to create a proxy for is not
   *     accessible.
   * @throws IOException if an exception occurred writing to the {@code dexCache} directory.
   * @throws UndeclaredThrowableException if the constructor for the base class to proxy throws a
   *     declared exception during construction.
   * @throws IllegalArgumentException if the handler is null, if the constructor argument types do
   *     not match the constructor argument values, or if no such constructor exists.
   */
  ...
  /**
   * Generate a proxy class. Note that new instances of this class will not automatically have an an
   * invocation handler, even if {@link #handler(InvocationHandler)} was called. The handler must be
   * set on each instance after it is created, using {@link #setInvocationHandler(Object,
   * InvocationHandler)}.
   */
  public Class<? extends T> buildProxyClass() throws IOException {
    ...
    dexMaker.declare(
        generatedType, generatedName + ".generated", PUBLIC, superType, getInterfacesAsTypeIds());
    if (sharedClassLoader) {
  ...
  private void getMethodsToProxy(
      Set<MethodSetEntry> sink, Set<MethodSetEntry> seenFinalMethods, Class<?> c) {
   ...
   *
   * <p>The purpose of this class is to override the {@link #equals(Object)} and {@link #hashCode()}
   * methods so we can use a {@link Set} to remove duplicate methods that are overrides of one
   * another. For these purposes, we consider two methods to be equal if they have the same name,
  ...
  private static class ProxiedClass<U> {
    ...
    /**
     * Class loader requested when the proxy class was generated. This might not be the class loader
     * of {@code clazz} as not all class loaders can be shared.
     */
