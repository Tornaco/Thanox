...
public final class Bits {
  ...
  /**
   * Returns whether or not the given bit set is empty, that is, whether no bit is set to {@code
   * true}.
   *
   * @param bits {@code non-null;} bit set to operate on
   ...
   * @return {@code true} if any bit is set to {@code true} in the indicated range
   */
   ...
   * @return {@code >= -1;} lowest-order bit set at or after {@code idx}, or {@code -1} if there is
   *     no appropriate bit index to return
   */
  ...
  /**
   * Finds the lowest-order bit set at or after the given index in the given {@code int}.
   *
  ...
  /**
   * Ors bit array {@code b} into bit array {@code a}. {@code a.length} must be greater than or
   * equal to {@code b.length}.
   *
   * @param a {@code non-null;} int array to be ored with other argument. This argument is modified.
   * @param b {@code non-null;} int array to be ored into {@code a}. This argument is not modified.
   */
