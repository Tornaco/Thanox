 ...
 *
 * <p><b>Note:</b> As per the {@link Output} interface, multi-byte writes all use little-endian
 * order.
 */
public final class ByteArrayAnnotatedOutput implements AnnotatedOutput, ByteOutput {
  ...
  /**
   * whether the instance is stretchy, that is, whether its array may be resized to increase
   * capacity
   */
  ...
  /**
   * Constructs an instance with a fixed maximum size. Note that the given array is the only one
   * that will be used to store data. In particular, no reallocation will occur in order to expand
   * the capacity of the resulting instance. Also, the constructed instance does not keep
  ...
  /**
   * Constructs a "stretchy" instance. The underlying array may be reallocated. The constructed
  ...
  /**
   * Constructs a "stretchy" instance with initial size {@code size}. The underlying array may be
   * reallocated. The constructed instance does not keep annotations by default.
   */
  ...
  /**
   * Gets the underlying {@code byte[]} of this instance, which may be larger than the number of
   * bytes written
   *
  ...
  /**
   * Constructs and returns a new {@code byte[]} that contains the written contents exactly (that
   * is, with no extra unwritten bytes at the end).
   *
  ...
  /**
   * Indicates that this instance should keep annotations. This method may be called only once per
   * instance, and only before any data has been written to the it.
   *
  ...
  /**
   * Finishes up annotation processing. This closes off any open annotations and removes annotations
   * that don't refer to written data.
   */
  ...
  /**
   * Reallocates the underlying array if necessary. Calls to this method should be guarded by a test
   * of {@link #stretchy}.
   *
  ...
  private static class Annotation {
     ...
     * @param end {@code >= start;} end of annotated range (exclusive) or {@code Integer.MAX_VALUE}
     *     if unclosed
