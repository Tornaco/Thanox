...
/**
 * Interface for a sink for binary output. This is similar to {@code java.util.DataOutput}, but no
 * {@code IOExceptions} are declared, and multibyte output is defined to be little-endian.
 */
public interface Output extends ByteOutput {
  /**
   * Gets the current cursor position. This is the same as the number of bytes written to this
   * instance.
   *
   ...
   * @throws RuntimeException thrown if {@code getCursor() != expectedCursor}
   */
  ...
  /**
   * Writes a DWARFv3-style unsigned LEB128 integer. For details, see the "Dalvik Executable Format"
   * document or DWARF v3 section 7.6.
   *
   ...
   *
   * @param bytes {@code non-null;} the array to write
   * @param offset {@code >= 0;} offset into {@code bytes} for the first byte to write
   * @param length {@code >= 0;} number of bytes to write
   */
  void write(byte[] bytes, int offset, int length);

  /**
   * Writes a {@code byte[]} to this instance. This is just a convenient shorthand for {@code
  ...
  /**
   * Adds extra bytes if necessary (with value {@code 0}) to force alignment of the output cursor as
   * given.
   *
