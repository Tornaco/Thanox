...
/**
 * This is a command line tool used by mainDexClasses script to build a boot dex classes list. First
 * argument of the command line is an archive, each class file contained in this archive is used to
 * identify a class that can be used during secondary dex installation, those class files are not
 * opened by this tool only their names matter. Other arguments must be zip files or directories,
 * they constitute in a classpath in with the classes named by the first argument will be searched.
 * Each searched class must be found. On each of this classes are searched for their dependencies to
 * other classes. The tool also browses for classes annotated by runtime visible annotations and
 * adds them to the list/ Finally the tools prints on standard output a list of class files names
 * suitable as content of the file argument --boot-dex-list of dx.
 */
public class MainDexListBuilder {
  ...
  /**
   * By default we force all classes annotated with runtime annotation to be kept in the boot dex
   * list. This option disable the workaround, limiting the index pressure in the boot dex but
   * exposing to the Dalvik resolution bug. The resolution bug occurs when accessing annotations of
   * a class that is not in the boot dex and one of the annotations as an enum parameter.
   *
    ...
    try {
      try {
        ...
        throw new IOException(
            "\"" + rootJar + "\" can not be read as a zip archive. (" + e.getMessage() + ")", e);
      }
  ...
  /** Returns a list of classes to keep. This can be passed to dx as a file with --boot-dex-list. */
