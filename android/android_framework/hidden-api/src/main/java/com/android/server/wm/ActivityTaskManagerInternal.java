...
public abstract class ActivityTaskManagerInternal {
  ...
  /**
   * Type for {@link #notifyAppTransitionStarting}: The transition was started because we drew the
   * splash screen.
   */
  public static final int APP_TRANSITION_SPLASH_SCREEN = 1;

  /**
   * Type for {@link #notifyAppTransitionStarting}: The transition was started because we all app
   * windows were drawn
   */
  public static final int APP_TRANSITION_WINDOWS_DRAWN = 2;

  /**
   * Type for {@link #notifyAppTransitionStarting}: The transition was started because of a timeout.
   */
  public static final int APP_TRANSITION_TIMEOUT = 3;

  /**
   * Type for {@link #notifyAppTransitionStarting}: The transition was started because of a we drew
   * a task snapshot.
   */
  ...
  /**
   * Sleep tokens cause the activity manager to put the top activity to sleep. They are used by
   * components such as dreams that may hide and block interaction with underlying activities.
   */
  public abstract static class SleepToken {
  ...
  public abstract void onLocalVoiceInteractionStarted(
      IBinder callingActivity, IVoiceInteractionSession mSession, IVoiceInteractor mInteractor);

  /**
   * Callback for window manager to let activity manager know that we are finally starting the app
   * transition;
   *
   ...
   * @param timestamp The time at which the app transition started in {@link
   *     SystemClock#uptimeMillis()} timebase.
   */
  public abstract void notifyAppTransitionStarting(SparseIntArray reasons, long timestamp);

  /**
   * Callback for window manager to let activity manager know that the app transition was cancelled.
   */
  ...
  /**
   * Returns the top activity from each of the currently visible stacks. The first entry will be the
   * focused activity.
   */
   ...
   *
   * <p>- DO NOT call it with the calling UID cleared. - All the necessary caller permission checks
   * must be done at callsites.
   *
   * @return error codes used by {@link IActivityManager#startActivity} and its siblings.
   */
  public abstract int startActivitiesAsPackage(
      String packageName, int userId, Intent[] intents, Bundle bOptions);
   ...
   * @param originatingPendingIntent PendingIntentRecord that originated this activity start or null
   *     if not originated by PendingIntent
   ...
   *
   * <p>- DO NOT call it with the calling UID cleared. - The caller must do the calling user ID
   * check.
   *
   * @return error codes used by {@link IActivityManager#startActivity} and its siblings.
   */
   ...
   *
   * @param callback Callback to run after activity visibilities have been reevaluated. This can be
   *     used from window manager so that when the callback is called, it's guaranteed that all apps
   *     have their visibility updated accordingly.
  ...
  /**
   * This enforces {@code func} can only be called if either the caller is Recents activity or has
   * {@code permission}.
   */
  ...
  /**
   * Set a uid that is allowed to bypass stopped app switches, launching an app whenever it wants.
   *
   * @param type Type of the caller -- unique string the caller supplies to identify itself and
   *     disambiguate with other calles.
  ...
  /**
   * Called when a user has been deleted. This can happen during normal device usage or just at
   * startup, when partially removed users are purged. Any state persisted by the ActivityManager
  ...
  /**
   * @return the activity token and IApplicationThread for the top activity in the task or null if
   *     there isn't a top activity with a valid process.
   */
  @Nullable
   ...
   * Default display always use primary home component. For Secondary displays, the home activity
   * must have category SECONDARY_HOME and then resolves according to the priorities listed below. -
   * If default home is not set, always use the secondary home defined in the config. - Use
   * currently selected primary home activity. - Use the activity in the same package as currently
   * selected primary home activity. If there are multiple activities matched, use first one. - Use
   * the secondary home defined in the config.
   */
  public abstract boolean startHomeOnDisplay(
      int userId, String reason, int displayId, boolean allowInstrumenting, boolean fromHomeKey);
  ...
  public abstract void handleAppDied(
      WindowProcessController wpc, boolean restarting, Runnable finishInstrumentationCallback);
  ...
  public abstract boolean onForceStopPackage(
      String packageName, boolean doit, boolean evenPersistent, int userId);
  /**
  ...
  public abstract void writeProcessesToProto(
      ProtoOutputStream proto, String dumpPackage, int wakeFullness, boolean testPssMode);
   ...
   */
  public abstract int finishTopCrashedActivities(WindowProcessController crashedApp, String reason);
  ...
  public abstract void onPackagesSuspendedChanged(String[] packages, boolean suspended, int userId);
  ...
  public abstract ActivityManager.TaskSnapshot getTaskSnapshotNoRestore(
