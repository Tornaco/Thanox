...
public final class EncodedValueReader {
  ...
  /**
   * Creates a new encoded value reader whose only value is the specified known type. This is useful
   * for encoded values without a type prefix, such as class_def_item's encoded_array or
  ...
  /**
   * Begins reading the elements of an array, returning the array's size. The caller must follow up
   * by calling a read method for each element in the array. For example, this reads a byte array:
   *
   * <pre>{@code
  ...
  /**
   * Begins reading the fields of an annotation, returning the number of fields. The caller must
   * follow up by making alternating calls to {@link #readAnnotationName()} and another read method.
   * For example, this reads an annotation whose fields are all bytes:
   *
   * <pre>{@code
  ...
  /**
   * Returns the type of the annotation just returned by {@link #readAnnotation()}. This method's
   * value is undefined unless the most recent call was to {@link #readAnnotation()}.
   */
  ...
  private void checkType(int expected) {
    if (peek() != expected) {
      throw new IllegalStateException(String.format("Expected %x but was %x", expected, peek()));
    }
  }
}
