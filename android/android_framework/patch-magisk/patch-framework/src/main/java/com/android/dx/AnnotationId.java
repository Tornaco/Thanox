 ...
 *
 * <p>Currently it is only targeting Class, Method, Field and Parameter because those are supported
 * by {@link com.android.dx.dex.file.AnnotationsDirectoryItem} so far.
 *
 * <p><strong>NOTE:</strong> So far it only supports adding method annotation. The annotation of
 * class, field and parameter will be implemented later.
 *
 * <p><strong>WARNING:</strong> The declared element of an annotation type should either have a
 * default value or be set a value via {@code AnnotationId.set(Element)}. Otherwise it will incur
...
public final class AnnotationId<D, V> {
   ...
   */
  public static <D, V> AnnotationId<D, V> get(
      TypeId<D> declaringType, TypeId<V> type, ElementType annotatedElement) {
  ...
  /**
   * Set an annotation element of this instance. If there is a preexisting element with the same
   * name, it will be replaced by this method.
   *
  ...
  public void addToMethod(DexMaker dexMaker, MethodId<?, ?> method) {
    ...
    if (!method.declaringType.equals(declaringType)) {
      throw new IllegalArgumentException(
          "Method" + method + "'s declaring type is inconsistent with" + this);
    }
  ...
  /**
   * A wrapper of <code>NameValuePair</code> represents a (name, value) pair used as the contents of
   * an annotation.
   *
   * <p>An {@code Element} instance is stored in {@code AnnotationId.elements} by calling {@code
   * AnnotationId.set(Element)}.
   *
   * <p><strong>WARNING: </strong> the name should be exact same as the annotation element declared
   * in the annotation type which is referred by field {@code AnnotationId.type},otherwise the
   * annotation will fail to add and {@code java.lang.reflect.Method.getAnnotations()} will return
   * nothing.
   */
