...
class AppDataDirGuesser {
  ...
  /**
   * Given the result of a ClassLoader.toString() call, process the result so that guessPath can use
   * it. There are currently two variants. For Android 4.3 and later, the string "DexPathList"
   * should be recognized and the array of dex path elements is parsed. for earlier versions, the
   * last nested array ('[' ... ']') is enclosing the string we are interested in.
   */
  ...
  private String getPathFromThisClassLoader(
