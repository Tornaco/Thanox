...
/**
 * Generates a <strong>D</strong>alvik <strong>EX</strong>ecutable (dex) file for execution on
 * Android. Dex files define classes and interfaces, including their member methods and fields,
 * executable code, and debugging information. They also define annotations, though this API
 * currently has no facility to create a dex file that contains annotations.
 *
 * <p>This library is intended to satisfy two use cases:
 *
 * <ul>
 *   <li><strong>For runtime code generation.</strong> By embedding this library in your Android
 *       application, you can dynamically generate and load executable code. This approach takes
 *       advantage of the fact that the host environment and target environment are both Android.
 *   <li><strong>For compile time code generation.</strong> You may use this library as a part of a
 *       compiler that targets Android. In this scenario the generated dex file must be installed on
 *       an Android device before it can be executed.
 * </ul>
 *
 * <h3>Example: Fibonacci</h3>
 *
 * To illustrate how this API is used, we'll use DexMaker to generate a class equivalent to the
 ...
 * }</pre>
 *
 * <p>We start by creating a {@link TypeId} to identify the generated {@code Fibonacci} class.
 * DexMaker identifies types by their internal names like {@code Ljava/lang/Object;} rather than
 * their Java identifiers like {@code java.lang.Object}.
 *
 * <pre>{@code
 * TypeId<?> fibonacci = TypeId.get("Lcom/google/dexmaker/examples/Fibonacci;");
 * }</pre>
 *
 * <p>Next we declare the class. It allows us to specify the type's source file for stack traces,
 * its modifiers, its superclass, and the interfaces it implements. In this case, {@code Fibonacci}
 * is a public class that extends from {@code Object}:
 *
 * <pre>{@code
 ...
 *
 * <p>To make it easier to go from our Java method to dex instructions, we'll manually translate it
 * to pseudocode fit for an assembler. We need to replace control flow like {@code if()} blocks and
 * {@code for()} loops with labels and branches. We'll also avoid performing multiple operations in
 * one statement, using local variables to hold intermediate values as necessary:
 *
 * <pre>{@code
 *   int constant1 = 1;
 *   int constant2 = 2;
 ...
 * }</pre>
 *
 * <p>We look up the {@code MethodId} for the method on the declaring type. This takes the method's
 * return type (possibly {@link TypeId#VOID}), its name and its parameters types. Next we declare
 * the method, specifying its modifiers by bitwise ORing constants from {@link Modifier}. The
 * declare call returns a {@link Code} object, which we'll use to define the method's instructions.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>One limitation of {@code DexMaker}'s API is that it requires all local variables to be created
 * before any instructions are emitted. Use {@link Code#newLocal newLocal()} to create a new local
 * variable. The method's parameters are exposed as locals using {@link Code#getParameter
 * getParameter()}. For non-static methods the {@code this} pointer is exposed using {@link
 * Code#getThis getThis()}. Here we declare all of the local variables that we'll need for our
 * {@code fib()} method:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>Notice that {@link Local} has a type parameter of {@code Integer}. This is useful for
 * generating code that works with existing types like {@code String} and {@code Integer}, but it
 * can be a hindrance when generating code that involves new types. For this reason you may prefer
 * to use raw types only and add {@code @SuppressWarnings("unsafe")} on your calling code. This will
 * yield the same result but you won't get IDE support if you make a type error.
 *
 * <p>We're ready to start defining our method's instructions. The {@link Code} class catalogs the
 * available instructions and their use.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>We're done defining the dex file. We just need to write it to the filesystem or load it into
 * the current process. For this example we'll load the generated code into the current process.
 * This only works when the current process is running on Android. We use {@link #generateAndLoad
 * generateAndLoad()} which takes the class loader that will be used as our generated code's parent
 * class loader. It also requires a directory where temporary files can be written.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * Finally we'll use reflection to lookup our generated class on its class loader and invoke its
 * {@code fib()} method:
 *
 * <pre>{@code
...
public final class DexMaker {
  ...
  public void declare(
    ...
    int supportedFlags =
        Modifier.PUBLIC | Modifier.FINAL | Modifier.ABSTRACT | AccessFlags.ACC_SYNTHETIC;
   ...
   *
   * @param flags a bitwise combination of {@link Modifier#PUBLIC}, {@link Modifier#PRIVATE}, {@link
   *     Modifier#PROTECTED}, {@link Modifier#STATIC}, {@link Modifier#FINAL}, {@link
   *     Modifier#VOLATILE}, and {@link Modifier#TRANSIENT}.
   * @param staticValue a constant representing the initial value for the static field, possibly
   *     null. This must be null if this field is non-static.
   */
   ...
   *
   * <p>If a class wants to call package private methods of another class they need to share a class
   * loader. One common case for this requirement is a mock class wanting to mock package private
   * methods of the original class.
   *
   * <p>If the classLoader is not a subclass of {@code dalvik.system.BaseDexClassLoader} this option
   * is ignored.
   *
    ...
    try {
      ...
      if (shareClassLoader) {
        if (!baseDexClassLoaderClass.isAssignableFrom(preferredClassLoader.getClass())) {
          if (!preferredClassLoader.getClass().getName().equals("java.lang.BootClassLoader")) {
            if (!didWarnNonBaseDexClassLoader) {
              System.err.println(
                  "Cannot share classloader as shared classloader '"
      ...
      if (markAsTrusted) {
        try {
          if (shareClassLoader) {
            preferredClassLoader
                .getClass()
                .getMethod("addDexPath", String.class, Boolean.TYPE)
                .invoke(preferredClassLoader, result.getPath(), true);
            return preferredClassLoader;
          } else {
            return (ClassLoader)
                baseDexClassLoaderClass
                    .getConstructor(
                        String.class, File.class, String.class, ClassLoader.class, Boolean.TYPE)
                    .newInstance(
                        result.getPath(),
                        dexCache.getAbsoluteFile(),
                        null,
                        preferredClassLoader,
                        true);
          }
          ...
          if (e.getCause() instanceof SecurityException) {
            if (!didWarnBlacklistedMethods) {
              System.err.println(
                  "Cannot allow to call blacklisted super methods. "
      ...
      if (shareClassLoader) {
        preferredClassLoader
            .getClass()
            .getMethod("addDexPath", String.class)
            .invoke(preferredClassLoader, result.getPath());
        return preferredClassLoader;
      } else {
        return (ClassLoader)
            Class.forName("dalvik.system.DexClassLoader")
                ...
                .newInstance(
                    result.getPath(), dexCache.getAbsolutePath(), null, preferredClassLoader);
      }
   ...
   *
   * The {@code dexCache} should be an application-private directory. If you pass a world-writable
   * directory like {@code /sdcard} a malicious app could inject code into your process. Most
   ...
   * }</pre>
   *
   * If the {@code dexCache} is null, this method will consult the {@code dexmaker.dexcache} system
   * property. If that exists, it will be used for the dex cache. If it doesn't exist, this method
   * will attempt to guess the application's private data directory as a last resort. If that fails,
   * this method will fail with an unchecked exception. You can avoid the exception by either
   * providing a non-null value or setting the system property.
   *
   * @param parent the parent ClassLoader to be used when loading our generated types (if set,
   *     overrides {@link #setSharedClassLoader(ClassLoader) shared class loader}.
   * @param dexCache the destination directory where generated and optimized dex files will be
   *     written. If null, this class will try to guess the application's private data dir.
   */
      ...
      if (property != null) {
        ...
        if (dexCache == null) {
          throw new IllegalArgumentException(
              "dexcache == null (and no default could be"
