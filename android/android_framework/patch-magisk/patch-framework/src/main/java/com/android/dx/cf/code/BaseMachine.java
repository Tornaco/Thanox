...
public abstract class BaseMachine implements Machine {
      ...
      if (!Merger.isPossiblyAssignableFrom(types.getType(i), args[i])) {
        throw new SimException(
            "at stack depth "
                + (size - 1 - i)
                + ", expected type "
                + types.getType(i).toHuman()
                + " but found "
                + args[i].getType().toHuman());
      }
    }
  }
  ...
  public final void popArgs(Frame frame, Type type) {
    ...
    if (!Merger.isPossiblyAssignableFrom(type, args[0])) {
      throw new SimException(
          "expected type " + type.toHuman() + " but found " + args[0].getType().toHuman());
    }
  }
  ...
  public final void popArgs(Frame frame, Type type1, Type type2) {
    ...
    if (!Merger.isPossiblyAssignableFrom(type1, args[0])) {
      throw new SimException(
          "expected type " + type1.toHuman() + " but found " + args[0].getType().toHuman());
    }

    if (!Merger.isPossiblyAssignableFrom(type2, args[1])) {
      throw new SimException(
          "expected type " + type2.toHuman() + " but found " + args[1].getType().toHuman());
    }
  }
  ...
  public final void popArgs(Frame frame, Type type1, Type type2, Type type3) {
    ...
    if (!Merger.isPossiblyAssignableFrom(type3, args[2])) {
      throw new SimException(
          "expected type " + type3.toHuman() + " but found " + args[2].getType().toHuman());
    }
  }
   ...
   *
   * @return {@code >= -1;} the salient local index or {@code -1} if none was set since the last
   *     time {@link #clearArgs} was called
   */
  ...
  /**
   * Gets the target local register spec of the current operation, if any. The local target spec is
   * the combination of the values indicated by a previous call to {@link #localTarget} with the
   * type of what should be the sole result set by a call to {@link #setResult} (or the combination
   * {@link #clearResult} then {@link #addResult}.
   *
   * @param isMove {@code true} if the operation being performed on the local is a move. This will
   *     cause constant values to be propagated to the returned local
   * @return {@code null-ok;} the salient register spec or {@code null} if no local target was set
   *     since the last time {@link #clearArgs} was called
   */
  protected final RegisterSpec getLocalTarget(boolean isMove) {
    ...
    if (resultCount != 1) {
      throw new SimException(
          "local target with " + ((resultCount == 0) ? "no" : "multiple") + " results");
    }
   ...
   *
   * <p><b>Note:</b> If there is more than one result value, the others may be added by using {@link
   * #addResult}.
   *
  ...
  /**
   * Gets the count of results. This throws an exception if results were never set. (Explicitly
   * clearing the results counts as setting them.)
   *
  ...
  /**
   * Stores the results of the latest operation into the given frame. If there is a local target
   * (see {@link #localTarget}), then the sole result is stored to that target; otherwise any
