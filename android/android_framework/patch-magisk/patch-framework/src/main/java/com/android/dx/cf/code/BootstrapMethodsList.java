...
/**
 * List of boot method entries, which are the contents of {@code BootstrapMethods} attributes.
 */
public class BootstrapMethodsList extends FixedSizeList {
   ...
   */
  public static BootstrapMethodsList concat(
      BootstrapMethodsList list1, BootstrapMethodsList list2) {
    if (list1 == EMPTY) {
      return list2;
    } else if (list2 == EMPTY) {
      return list1;
   ...
   *
   * @param n {@code >= 0, < size();} which element
   * @param declaringClass {@code non-null;} the class declaring boot method.
   * @param bootstrapMethodHandle {@code non-null;} the boot method handle
   * @param arguments {@code non-null;} the arguments of the boot method
   */
  public void set(
      int n,
