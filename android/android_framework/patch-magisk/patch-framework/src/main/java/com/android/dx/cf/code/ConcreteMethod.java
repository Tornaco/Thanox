...
/**
 * Container for all the giblets that make up a concrete Java bytecode method. It implements {@link
 * Method}, so it provides all the original access (by delegation), but it also constructs and keeps
 * useful versions of stuff extracted from the method's {@code Code} attribute.
 */
public final class ConcreteMethod implements Method {
  ...
  public ConcreteMethod(Method method, ClassFile classFile, boolean keepLines, boolean keepLocals) {
    ...
    if (keepLines) {
      for (AttLineNumberTable lnt =
              (AttLineNumberTable) codeAttribs.findFirst(AttLineNumberTable.ATTRIBUTE_NAME);
          lnt != null;
    ...
    if (keepLocals) {
       ...
       */
      for (AttLocalVariableTable lvt =
              (AttLocalVariableTable) codeAttribs.findFirst(AttLocalVariableTable.ATTRIBUTE_NAME);
          lvt != null;
      ...
      for (AttLocalVariableTypeTable lvtt =
              (AttLocalVariableTypeTable)
                  codeAttribs.findFirst(AttLocalVariableTypeTable.ATTRIBUTE_NAME);
          lvtt != null;
  ...
  public final boolean isDefaultOrStaticInterfaceMethod() {
    return (classFile.getAccessFlags() & AccessFlags.ACC_INTERFACE) != 0 && !getNat().isClassInit();
  }
  ...
  public SourcePosition makeSourcePosistion(int offset) {
    return new SourcePosition(getSourceFile(), offset, lineNumbers.pcToLine(offset));
  }
}
