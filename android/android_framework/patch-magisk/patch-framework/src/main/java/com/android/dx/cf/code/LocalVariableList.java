...
/**
 * List of "local variable" entries, which are the contents of {@code LocalVariableTable} and {@code
 * LocalVariableTypeTable} attributes, as well as combinations of the two.
 */
public final class LocalVariableList extends FixedSizeList {
  ...
  /**
   * Returns an instance which is the concatenation of the two given instances. The result is
   * immutable.
   *
  ...
  /**
   * Returns an instance which is the result of merging the two given instances, where one instance
   * should have only type descriptors and the other only type signatures. The merged result is
   * identical to the one with descriptors, except that any element whose {name, index, start,
   * length} matches an element in the signature list gets augmented with the corresponding
   * signature. The result is immutable.
   *
  ...
  public static LocalVariableList mergeDescriptorsAndSignatures(
      LocalVariableList descriptorList, LocalVariableList signatureList) {
   ...
   * Sets the item at the given index.
   *
   * <p><b>Note:</b> At least one of {@code descriptor} or {@code signature} must be passed as
   * non-null.
   *
   * @param n {@code >= 0, < size();} which element
   * @param startPc {@code >= 0;} the start pc of this variable's scope
   * @param length {@code >= 0;} the length (in bytecodes) of this variable's scope
  ...
  /**
   * Gets the local variable information in this instance which matches the given {@link
   * Item} in all respects but the type descriptor and
   * signature, if any.
   *
   * @param item {@code non-null;} local variable information to match
   * @return {@code null-ok;} the corresponding local variable information stored in this instance,
   *     or {@code null} if there is no matching information
   */
   ...
   * <b>Note:</b> In standard classfiles, a variable's start point is listed as the address of the
   * instruction <i>just past</i> the one that sets the variable.
   *
   * @param pc {@code >= 0;} the address to look up
   * @param index {@code >= 0;} the local variable index
   * @return {@code null-ok;} the associated local variable information, or {@code null} if none is
   *     known
   */
  ...
  public static class Item {
    ...
    /**
     * Gets the variable's type signature. Private because if you need this, you want getLocalItem()
     * instead.
     *
    ...
    /**
     * Gets the variable's type descriptor. This is a convenient shorthand for {@code
    ...
    /**
     * Constructs and returns an instance which is identical to this one, except that the signature
    ...
    public Item withSignature(CstString newSignature) {
      return new Item(startPc, length, name, descriptor, newSignature, index);
    }
     ...
     * @return {@code true} iff this instance matches {@code pc} and {@code index}
     */
    public boolean matchesPcAndIndex(int pc, int index) {
      return (index == this.index) && (pc >= startPc) && (pc < (startPc + length));
    }

    /**
     * Gets whether this instance matches (describes) the given other instance exactly in all fields
     * except type descriptor and type signature.
     *
