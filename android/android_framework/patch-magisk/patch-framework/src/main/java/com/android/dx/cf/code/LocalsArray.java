 ...
 *
 * <p><b>Note:</b> For the most part, the documentation for this class ignores the distinction
 * between {@link Type} and {@link TypeBearer}.
 */
public abstract class LocalsArray extends MutabilityControl implements ToHuman {
  ...
  /**
   * Replaces all the occurrences of the given uninitialized type in this array with its initialized
   * equivalent.
   *
  ...
  /**
   * Sets the type stored at the given local index. If the given type is category-2, then (a) the
   * index must be at least two less than {@link #getMaxLocals} and (b) the next index gets
   * invalidated by the operation. In case of either category, if the <i>previous</i> local contains
   * a category-2 value, then it too is invalidated by this operation.
   *
  ...
  /**
   * Sets the type for the local indicated by the given register spec to that register spec (which
   * includes type and optional name information). This is identical to calling {@code
  ...
  /**
   * Gets the type stored at the given local index, or {@code null} if the given local is
  ...
  /**
   * Gets the type stored at the given local index, only succeeding if the given local contains a
   * valid type (though it is allowed to be an uninitialized instance).
   *
   * @param idx {@code >= 0, < getMaxLocals();} which local
   * @return {@code non-null;} the type of value stored in that local
   * @throws SimException thrown if {@code idx} is valid, but the contents are invalid
   */
  public abstract TypeBearer get(int idx);

  /**
   * Gets the type stored at the given local index, which is expected to be an initialized
   * category-1 value.
   *
   * @param idx {@code >= 0, < getMaxLocals();} which local
   * @return {@code non-null;} the type of value stored in that local
   * @throws SimException thrown if {@code idx} is valid, but one of the following holds: (a) the
   *     local is invalid; (b) the local contains an uninitialized instance; (c) the local contains
   *     a category-2 value
   */
   ...
   * @return {@code non-null;} the type of value stored in that local
   * @throws SimException thrown if {@code idx} is valid, but one of the following holds: (a) the
   *     local is invalid; (b) the local contains a category-1 value
   */
  public abstract TypeBearer getCategory2(int idx);

  /**
   * Merges this instance with {@code other}. If the merged result is the same as this instance,
   * then this is returned (not a copy).
   *
  ...
  /**
   * Merges this instance with a {@code LocalsSet} from a subroutine caller. To be used when merging
   * in the first block of a subroutine.
   *
   * @param other {@code other non-null;} another LocalsArray. The final locals state of a
   *     subroutine caller.
  ...
  /**
   * Gets the locals set appropriate for the current execution context. That is, if this is a {@code
   * OneLocalsArray} instance, then return {@code this}, otherwise return {@code LocalsArraySet}'s
   * primary.
   *
