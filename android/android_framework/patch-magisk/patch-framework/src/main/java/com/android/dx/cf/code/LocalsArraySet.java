...
/**
 * Representation of a set of local variable arrays, with Java semantics. This peculiar case is to
 * support in-method subroutines, which can have different locals sets for each caller.
 *
 * <p><b>Note:</b> For the most part, the documentation for this class ignores the distinction
 * between {@link com.android.dx.rop.type.Type} and {@link com.android.dx.rop.type.TypeBearer}.
 */
public class LocalsArraySet extends LocalsArray {

  /**
   * The primary LocalsArray represents the locals as seen from the subroutine itself, which is the
   * merged representation of all the individual locals states.
   */
  private final OneLocalsArray primary;

  /**
   * Indexed by label of caller block: the locals specific to each caller's invocation of the
   * subroutine.
   */
   ...
   * @param secondaries {@code non-null;} secondaries set, indexed by subroutine caller label.
   */
   ...
   * @return {@code non-null;} this instance if merge was a no-op, or new merged instance.
   */
  ...
  /**
   * Gets the {@code LocalsArray} instance for a specified subroutine caller label, or null if label
   * has no locals associated with it.
   *
  ...
  /**
   * Returns a LocalsArray instance representing the locals state that should be used when returning
   * to a subroutine caller.
   *
   * @param subLabel {@code >= 0;} A calling label of a subroutine
   * @return {@code null-ok;} an instance for this subroutine, or null if subroutine is not in this
   *     set.
   */
