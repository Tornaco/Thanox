...
public final class Merger {
  ...
  /**
   * Merges two locals arrays. If the merged result is the same as the first argument, then return
   * the first argument (not a copy).
   *
  ...
  /**
   * Merges two stacks. If the merged result is the same as the first argument, then return the
   * first argument (not a copy).
   *
  ...
  /**
   * Returns whether the given supertype is possibly assignable from the given subtype. This takes
   * into account primitiveness, int-likeness, known-nullness, and array dimensions, but does not
   * assume anything about class hierarchy other than that the type {@code Object} is the supertype
   * of all reference types and all arrays are assignable to {@code Serializable} and {@code
   * Cloneable}.
   *
   * @param supertypeBearer {@code non-null;} the supertype
   * @param subtypeBearer {@code non-null;} the subtype
   */
  public static boolean isPossiblyAssignableFrom(
      TypeBearer supertypeBearer, TypeBearer subtypeBearer) {
