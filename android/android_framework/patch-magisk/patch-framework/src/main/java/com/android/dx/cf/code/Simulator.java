 ...
 *
 * <p><b>Note:</b> This class is not thread-safe. If multiple threads need to use a single instance,
 * they must synchronize access explicitly between themselves.
 */
public class Simulator {
   ...
   *
   * <p>The interesting cases here have to do with object arrays, <code>byte[]</code>s, <code>
   * boolean[]</code>s, and known-nulls.
   *
   * <p>In the case of arrays of objects, we want to narrow the type to the actual array present on
   * the stack, as long as what is present is an object type. Similarly, due to a quirk of the
   * original bytecode representation, the instructions for dealing with <code>byte[]</code> and
   * <code>boolean[]</code> are undifferentiated, and we aim here to return whichever one was
   * actually present on the stack.
   *
   * <p>In the case where there is a known-null on the stack where an array is expected, our
   * behavior depends on the implied type of the instruction. When the implied type is a reference,
   * we don't attempt to infer anything, as we don't know the dimension of the null constant and
   * thus any explicit inferred type could be wrong. When the implied type is a primitive, we fall
   * back to the implied type of the instruction. Due to the quirk described above, this means that
   * source code that uses <code>boolean[]</code> might get translated surprisingly -- but correctly
   * -- into an instruction that specifies a <code>byte[]</code>. It will be correct, because should
   * the code actually execute, it will necessarily throw a <code>NullPointerException</code>, and
   * it won't matter what opcode variant is used to achieve that result.
   *
   * @param impliedType {@code non-null;} type implied by the instruction; is <i>not</i> an array
   *     type
   * @param foundArrayType {@code non-null;} type found on the stack; is either an array type or a
   *     known-null
   * @return {@code non-null;} the array type that should be required in this context
   */
  ...
  /**
   * Simulates the effect of executing the given basic block. This modifies the passed-in frame to
  ...
  /**
   * Simulates the effect of the instruction at the given offset, by making appropriate calls on the
   * given frame.
   *
  ...
  private void checkConstMethodHandleSupported(Constant cst) throws SimException {
    if (!dexOptions.apiIsSupported(DexFormat.API_CONST_METHOD_HANDLE)) {
      fail(
          String.format(
              "invalid constant type %s requires --min-sdk-version >= %d " + "(currently %d)",
              cst.typeName(), DexFormat.API_CONST_METHOD_HANDLE, dexOptions.minSdkVersion));
    }
  }
  ...
  private void checkInvokeDynamicSupported(int opcode) throws SimException {
    if (!dexOptions.apiIsSupported(DexFormat.API_METHOD_HANDLES)) {
      fail(
          String.format(
              "invalid opcode %02x - invokedynamic requires "
    ...
    if (softFail) {
      ...
      String reason =
          String.format(
              ...
              invokeKind,
              callee.getDefiningClass().toHuman(),
              callee.getNat().toHuman(),
              DexFormat.API_INVOKE_INTERFACE_METHODS,
    ...
    if (!dexOptions.apiIsSupported(DexFormat.API_DEFINE_INTERFACE_METHODS)) {
      String reason =
          String.format(
              ...
              DexFormat.API_DEFINE_INTERFACE_METHODS,
              dexOptions.minSdkVersion,
              declaredMethod.getDefiningClass().toHuman(),
              declaredMethod.getNat().toHuman());
      warn(reason);
    }
  }
  ...
  private void fail(String reason) {
    String message =
        String.format(
            "ERROR in %s.%s: %s",
            method.getDefiningClass().toHuman(), method.getNat().toHuman(), reason);
  ...
  private class SimVisitor implements BytecodeArray.Visitor {
    /**
     * {@code non-null;} machine instance to use (just to avoid excessive cross-object field access)
     */
    private final Machine machine;

    /** {@code null-ok;} frame to use; set with each call to {@link Simulator#simulate} */
    private Frame frame;
        ...
        case ByteOps.ARRAYLENGTH:
          {
            ...
            if (!arrayType.isArrayOrKnownNull()) {
              fail("type mismatch: expected array type but encountered " + arrayType.toHuman());
            }
    ...
    /**
     * Checks whether the prototype is compatible with returning the given type, and throws if not.
     *
    ...
    private void checkReturnType(Type encountered) {
      ...
      if (!Merger.isPossiblyAssignableFrom(returnType, encountered)) {
        fail(
            "return type mismatch: prototype "
                + "indicates "
                + returnType.toHuman()
