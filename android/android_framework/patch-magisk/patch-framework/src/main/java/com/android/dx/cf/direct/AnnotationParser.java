...
public final class AnnotationParser {
  ...
  /** {@code non-null;} cursor for use when informing the observer of what was parsed */
   ...
   *
   * @param visibility {@code non-null;} visibility of the parsed annotations
   * @return {@code non-null;} the list of annotations read from the attribute data
   */
   ...
   *
   * @param visibility {@code non-null;} visibility of the parsed annotations
   * @return {@code non-null;} the list of annotation lists read from the attribute data
   */
  ...
  /**
   * Helper for {@link #parseValue}, which parses a constant reference and returns the referred-to
   * constant value.
   *
   * @return {@code non-null;} the parsed value
   */
  private Constant parseConstant() throws IOException {
    ...
    if (observer != null) {
      String human =
          (value instanceof CstString) ? ((CstString) value).toQuoted() : value.toHuman();
  ...
  /**
   * Helper which indicates that some bytes were just parsed. This should only be used (for
   * efficiency sake) if the parse is known to be observed.
   *
  ...
  /**
   * Convenience wrapper that simply calls through to {@code observer.changeIndent()}.
   *
