...
abstract /*package*/ class MemberListParser {
  ...
  /**
   * {@code >= -1;} the end offset of this list in the byte array of the classfile, or {@code -1} if
   * not yet parsed
   */
  ...
  /**
   * Gets the human-oriented name for what this instance is parsing. Subclasses must override this
   * method.
   *
  ...
  /**
   * Gets the human-oriented string for the given access flags. Subclasses must override this
   * method.
   *
  ...
  /**
   * Gets the {@code CTX_*} constant to use when parsing attributes. Subclasses must override this
   * method.
   *
   * @return {@code non-null;} the human oriented name
   */
   ...
   * @param nat the interpreted name and type (based on the two {@code *_index} fields)
   * @param attributes list of parsed attributes
   * @return {@code non-null;} the constructed member
   */
  protected abstract Member set(int n, int accessFlags, CstNat nat, AttributeList attributes);
    ...
    for (int i = 0; i < count; i++) {
      try {
        ...
        AttributeListParser parser =
            new AttributeListParser(cf, attributeContext, at, attributeFactory);
