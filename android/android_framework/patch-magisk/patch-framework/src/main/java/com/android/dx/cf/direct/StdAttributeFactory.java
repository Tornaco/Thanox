...
/**
 * Standard subclass of {@link AttributeFactory}, which knows how to parse all the standard
 * attribute types.
 */
public class StdAttributeFactory extends AttributeFactory {
  ...
  private Attribute bootstrapMethods(
      DirectClassFile cf, int offset, int length, ParseObserver observer) {
    if (length < 2) {
    ...
    BootstrapMethodsList methods =
        parseBootstrapMethods(
            bytes, cf.getConstantPool(), cf.getThisClass(), numMethods, offset, length, observer);
  ...
  private Attribute localVariableTable(
      DirectClassFile cf, int offset, int length, ParseObserver observer) {
    if (length < 2) {
      return throwSeverelyTruncated();
    }
    ...
    LocalVariableList list =
        parseLocalVariables(
            bytes.slice(offset + 2, offset + length), cf.getConstantPool(), observer, count, false);
  ...
  private Attribute runtimeInvisibleAnnotations(
      DirectClassFile cf, int offset, int length, ParseObserver observer) {
    if (length < 2) {
      throwSeverelyTruncated();
    }
  ...
  private Attribute runtimeInvisibleParameterAnnotations(
      DirectClassFile cf, int offset, int length, ParseObserver observer) {
    if (length < 2) {
      throwSeverelyTruncated();
    }
  ...
  private BootstrapMethodsList parseBootstrapMethods(
      ByteArray bytes,
    ...
    for (int methodIndex = 0; methodIndex < numMethods; ++methodIndex) {
      ...
      for (int argIndex = 0; argIndex < numArguments; ++argIndex, offset += 2, length -= 2) {
        ...
        if (observer != null) {
          observer.parsed(
              bytes, offset, 2, "bootstrap_arguments[" + argIndex + "]" + Hex.u2(argumentRef));
        }
