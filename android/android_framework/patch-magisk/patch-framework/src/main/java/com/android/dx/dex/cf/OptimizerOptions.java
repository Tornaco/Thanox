...
public class OptimizerOptions {
  /**
   * {@code null-ok;} hash set of class name + method names that should be optimized. {@code null}
   * if this constraint was not specified on the command line
   */
  private HashSet<String> optimizeList;

  /**
   * {@code null-ok;} hash set of class name + method names that should NOT be optimized. null if
   * this constraint was not specified on the command line
   */
   ...
   */
  public void compareOptimizerStep(
    ...
    System.err.printf(
        "optimize step regs:(%d/%d/%.2f%%)" + " insns:(%d/%d/%.2f%%)\n",
        rmeth.getBlocks().getRegCount(),
        skipRopMethod.getBlocks().getRegCount(),
        100.0
            * ((skipRopMethod.getBlocks().getRegCount() - rmeth.getBlocks().getRegCount())
