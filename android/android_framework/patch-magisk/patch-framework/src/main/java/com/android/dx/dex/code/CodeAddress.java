...
/**
 * Pseudo-instruction which is used to track an address within a code array. Instances are used for
 * such things as branch targets and exception handler ranges. Its code size is zero, and so
 * instances do not in general directly wind up in any output (either human-oriented or binary
 * file).
 */
public final class CodeAddress extends ZeroSizeInsn {
   ...
   *
   * <p>When a prefix is added to an instruction (for example, to move a value from a high register
   * to a low register), this determines whether this {@code CodeAddress} will point to the prefix,
   * or to the instruction itself.
   *
   * <p>If bindsClosely is true, the address will point to the instruction itself, otherwise it will
   * point to the prefix (if any)
   *
