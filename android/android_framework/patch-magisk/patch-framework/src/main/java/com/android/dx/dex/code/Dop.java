...
public final class Dop {
  ...
  /**
   * {@code Opcodes.isValid();} what opcode (by number) to try next when attempting to match an
   * opcode to particular arguments; {@code Opcodes.NO_NEXT} to indicate that this is the last
   ...
   * @param nextOpcode {@code Opcodes.isValid();} what opcode (by number) to try next when
   *     attempting to match an opcode to particular arguments; {@code Opcodes.NO_NEXT} to indicate
   *     that this is the last opcode to try in a particular chain
   * @param format {@code non-null;} the instruction format
   * @param hasResult whether the opcode has a result register; if so it is always the first
   *     register
   */
  ...
  /**
   * Gets the opcode family. The opcode family is the unmarked (no "/...") opcode that has
   * equivalent semantics to this one.
   *
   ...
   * This returns {@code Opcodes.NO_NEXT} to indicate that this is the last opcode to try in a
   * particular chain.
   *
   * @return {@code Opcodes.MIN_VALUE..Opcodes.MAX_VALUE;} the opcode value
   */
  ...
  /**
   * Gets the opcode for the opposite test of this instance. This is only valid for opcodes which
   * are in fact tests.
   *
