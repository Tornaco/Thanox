...
/**
 * Combination instruction which turns into a variable number of {@code move*} instructions to move
 * a set of registers into registers starting at {@code 0} sequentially. This is used in translating
 * an instruction whose register requirements cannot be met using a straightforward choice of a
 * single opcode.
 */
public final class HighRegisterPrefix extends VariableSizeInsn {
  ...
  private static SimpleInsn moveInsnFor(RegisterSpec src, int destIndex) {
    return DalvInsn.makeMove(
        SourcePosition.NO_INFO, RegisterSpec.make(destIndex, src.getType()), src);
  }
  ...
  /**
   * Helper for {@link #codeSize} and {@link #writeTo} which sets up {@link #insns} if not already
   * done.
   */
