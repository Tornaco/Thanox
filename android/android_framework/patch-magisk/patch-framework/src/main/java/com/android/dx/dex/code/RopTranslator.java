...
/**
 * Translator from {@link RopMethod} to {@link DalvCode}. The {@link #translate} method is the thing
 * to call on this class.
 */
public final class RopTranslator {
  ...
  /**
   * true if the parameters to this method happen to be in proper order at the end of the frame (as
   * the optimizer emits them)
   */
   ...
   * @param positionInfo how much position info to preserve; one of the static constants in {@link
   *     PositionList}
   * @param locals {@code null-ok;} local variable information to use
   * @param paramSize size, in register units, of all the parameters to this method
  ...
  /**
   * Checks to see if the move-param instructions that occur in this method happen to slot the
   * params in an order at the top of the stack frame that matches dalvik's calling conventions.
   * This will alway result in "true" for methods that have run through the SSA optimizer.
   *
   * @param paramSize size, in register units, of all the parameters to this method
   */
  ...
  /**
   * Gets the complete register list (result and sources) out of a given rop instruction. For insns
   * that are commutative, have two register sources, and have a source equal to the result, place
   * that source first.
   *
   ...
   * @param nextLabel {@code >= -1;} the next block that will be processed, or {@code -1} if there
   *     is no next block
   */
  ...
  private class TranslationVisitor implements Insn.Visitor {
    ...
    /**
     * {@code null-ok;} code address for the salient last instruction of the block (used before
     * switches and throwing instructions)
     */
     ...
     * @param lastAddress {@code non-null;} code address for the salient last instruction of the
     *     block
     */
    ...
    public void visitPlainInsn(PlainInsn insn) {
      ...
      switch (rop.getBranchingness()) {
        ...
        case Rop.BRANCH_GOTO:
          {
            /*
             * Code in the boot translation loop will emit a
    ...
    /**
     * Looks forward to the current block's primary successor, returning the RegisterSpec of the
     * result of the move-result-pseudo at the top of that block or null if none.
     *
     * @return {@code null-ok;} result of move-result-pseudo at the beginning of primary successor
     */
  ...
  /** Instruction visitor class for doing instruction translation with local variable tracking */
  private class LocalVariableAwareTranslationVisitor extends TranslationVisitor {
    ...
    /**
     * Adds a {@link LocalStart} to the output if the given instruction in fact introduces a local
     * variable.
     *
     * @param insn {@code non-null;} instruction in question
     */
