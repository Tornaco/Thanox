...
/**
 * Pseudo-instruction which holds switch data. The switch data is a map of values to target
 * addresses, and this class writes the data in either a "packed" or "sparse" form.
 */
public final class SwitchData extends VariableSizeInsn {
   ...
   * @param user {@code non-null;} address representing the instruction that uses this instance
   * @param cases {@code non-null;} sorted list of switch cases (keys)
   * @param targets {@code non-null;} corresponding list of code addresses; the branch target for
   *     each case
   */
  public SwitchData(
      SourcePosition position, CodeAddress user, IntList cases, CodeAddress[] targets) {
   ...
   * @return {@code >= -1;} the packed table size or {@code -1} if the cases couldn't possibly be
   *     represented as a packed table
   */
