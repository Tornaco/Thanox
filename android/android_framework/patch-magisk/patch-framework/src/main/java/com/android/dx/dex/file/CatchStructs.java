...
/**
 * List of exception handlers (tuples of covered range, catch type, handler address) for a
 * particular piece of code. Instances of this class correspond to a {@code try_item[]} and a {@code
 * catch_handler_item[]}.
 */
public final class CatchStructs {
  ...
  /** {@code null-ok;} the underlying table; set in {@link #finishProcessingIfNecessary} */
  private CatchTable table;

  /** {@code null-ok;} the encoded handler list, if calculated; set in {@link #encode} */
  private byte[] encodedHandlers;

  /** length of the handlers header (encoded size), if known; used for annotation */
  private int encodedHandlerHeaderSize;

  /**
   * {@code null-ok;} map from handler lists to byte offsets, if calculated; set in {@link #encode}
   */
  ...
  /**
   * Helper for {@link #annotateEntries} to annotate a catch handler list while consuming it.
   *
   ...
   */
  private static void annotateAndConsumeHandlers(
  ...
  /**
   * Helper method to annotate or simply print the exception handlers. Only one of {@code printTo}
   * or {@code annotateTo} should be non-null.
   *
   * @param prefix {@code non-null;} prefix for each line
   * @param printTo {@code null-ok;} where to print to
  ...
  private void annotateEntries(String prefix, PrintWriter printTo, AnnotatedOutput annotateTo) {
    ...
    annotateTo.annotate(
        encodedHandlerHeaderSize, subPrefix + "size: " + Hex.u2(handlerOffsets.size()));
    ...
    for (Map.Entry<CatchHandlerList, Integer> mapping : handlerOffsets.entrySet()) {
      ...
      if (lastList != null) {
        annotateAndConsumeHandlers(
            lastList, lastOffset, offset - lastOffset, subPrefix, printTo, annotateTo);
      }
    ...
    annotateAndConsumeHandlers(
        lastList, lastOffset, encodedHandlers.length - lastOffset, subPrefix, printTo, annotateTo);
  }
}
