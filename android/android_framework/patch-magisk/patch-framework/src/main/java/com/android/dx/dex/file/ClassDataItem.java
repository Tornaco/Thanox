...
/**
 * Representation of all the parts of a Dalvik class that are generally "inflated" into an in-memory
 * representation at runtime. Instances of this class are represented in a compact streamable form
 * in a {@code dex} file, as opposed to a random-access form.
 */
public final class ClassDataItem extends OffsettedItem {
   ...
   *
   * @param thisClass {@code non-null;} what class this data is for, just for listing generation
   */
  ...
  /**
   * Helper for {@link #encodeOutput}, which writes out the given size value, annotating it as well
  ...
  /**
   * Helper for {@link #encodeOutput}, which writes out the given list. It also annotates the items
   * (if any and if annotations are enabled).
   *
   * @param file {@code non-null;} file this instance is part of
   * @param out {@code non-null;} where to write to
   * @param label {@code non-null;} the label for the purposes of annotation
  ...
  private static void encodeList(
      DexFile file, AnnotatedOutput out, String label, ArrayList<? extends EncodedMember> list) {
    int size = list.size();
    int lastIndex = 0;
   ...
   *
   * @return {@code true} if this instance is empty or {@code false} if at least one element has
   *     been added to it
   */
  ...
  /**
   * Gets all the methods in this class. The returned list is not linked in any way to the
   * underlying lists contained in this instance, but the objects contained in the list are shared.
   *
  ...
  /**
   * Gets a {@link CstArray} corresponding to {@link #staticValues} if it contains any non-zero
   * non-{@code null} values.
   *
   * @return {@code null-ok;} the corresponding constant or {@code null} if there are no values to
   *     encode
   */
