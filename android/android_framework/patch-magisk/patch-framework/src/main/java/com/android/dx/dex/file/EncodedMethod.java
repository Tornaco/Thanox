...
public final class EncodedMethod extends EncodedMember implements Comparable<EncodedMethod> {
  ...
  /**
   * {@code null-ok;} code for the method, if the method is neither {@code abstract} nor {@code
   * native}
   */
   ...
   * @param code {@code null-ok;} code for the method, if it is neither {@code abstract} nor {@code
   *     native}
   * @param throwsList {@code non-null;} list of possibly-thrown exceptions, just used in generating
   *     debugging output (listings)
   */
   ...
   * {@inheritDoc}
   *
   * <p><b>Note:</b> This compares the method constants only, ignoring any associated code, because
   * it should never be the case that two different items with the same method constant ever appear
   * in the same list (or same file, even).
   */
  @Override
