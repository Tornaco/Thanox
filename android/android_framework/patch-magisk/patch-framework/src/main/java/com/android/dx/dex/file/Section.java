...
/**
 * A section of a {@code .dex} file. Each section consists of a list of items of some sort or other.
 */
public abstract class Section {
  ...
  /** {@code > 0;} alignment requirement for the final output; must be a power of 2 */
  private final int alignment;

  /**
   * {@code >= -1;} offset from the start of the file to this part, or {@code -1} if not yet known
   */
   ...
   *
   * @param name {@code null-ok;} the name of this instance, for annotation purposes
   * @param file {@code non-null;} file that this instance is part of
   * @param alignment {@code > 0;} alignment requirement for the final output; must be a power of 2
   */
   ...
   * @throws IllegalArgumentException thrown if {@code alignment} isn't a positive power of 2
   */
  ...
  /**
   * Gets the offset from the start of the file to this part. This throws an exception if the offset
   ...
   *
   * @param fileOffset {@code >= 0;} the desired offset from the start of the file where this for
   *     this instance
   ...
   * This is only valid to call once this instance has been assigned a file offset (via {@link
   * #setFileOffset}).
   *
   ...
   * #setFileOffset}).
   *
   * <p><b>Note:</b> Subclasses must implement this as appropriate for their contents.
   *
   ...
   * particularly adding stuff to other sections. This method may only be called once per instance;
  ...
  /**
   * Gets the collection of all the items in this section. It is not valid to attempt to change the
   * returned list.
   *
  ...
  /** Does the boot work of {@link #prepare}. */
  ...
  /**
   * Writes this instance to the given raw data object. This gets called by {@link #writeTo} after
   * aligning the cursor of {@code out} and verifying that either the assigned file offset matches
   * the actual cursor {@code out} or that the file offset was not previously assigned, in which
