...
public final class Opcodes {
  /**
   * pseudo-opcode used for nonstandard format payload "instructions". TODO: Retire this concept,
   * and start treating the payload instructions more like the rest.
   */
  ...
  /**
   * Determines if the given opcode has the right "shape" to be valid. This includes the range
   * {@code 0x01..0xfe}, the range {@code 0x00ff..0xffff} where the low-order byte is either {@code
   * 0} or {@code 0xff}, and the special opcode values {@code SPECIAL_FORMAT} and {@code NO_NEXT}.
   * Note that not all of the opcode values that pass this test are in fact used. This method is
   * meant to perform a quick check to reject blatantly wrong values (e.g. when validating
   * arguments).
   *
   * @param opcode the opcode value
   * @return {@code true} iff the value has the right "shape" to be possibly valid
   */
  ...
  /**
   * Gets the opcode out of an opcode unit, the latter of which may also include one or more
   * argument values.
   *
