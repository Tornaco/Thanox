...
public final class IndexMap {
  ...
  public SortableType adjust(SortableType sortableType) {
    return new SortableType(
        sortableType.getDex(), sortableType.getIndexMap(), adjust(sortableType.getClassDef()));
  }
  ...
  public EncodedValue adjustEncodedArray(EncodedValue encodedArray) {
    ByteArrayAnnotatedOutput out = new ByteArrayAnnotatedOutput(32);
    new EncodedValueTransformer(out)
        .transformArray(new EncodedValueReader(encodedArray, ENCODED_ARRAY));
    return new EncodedValue(out.toByteArray());
  }
    ...
    public void transform(EncodedValueReader reader) {
      ...
      switch (reader.peek()) {
        ...
        case ENCODED_METHOD_HANDLE:
          EncodedValueCodec.writeUnsignedIntegralValue(
              out, ENCODED_METHOD_HANDLE, adjustMethodHandle(reader.readMethodHandle()));
          break;
        case ENCODED_STRING:
          EncodedValueCodec.writeUnsignedIntegralValue(
