...
/**
 * Name and structure of a type. Used to order types such that each type is preceded by its
 * supertype and implemented interfaces.
 */
final class SortableType {
  public static final Comparator<SortableType> NULLS_LAST_ORDER =
      new Comparator<SortableType>() {
        @Override
  ...
  /**
   * Assigns this type's depth if the depths of its supertype and implemented interfaces are known.
   * Returns false if the depth couldn't be computed yet.
   */
  public boolean tryAssignDepth(SortableType[] types) {
    ...
    if (classDef.getSupertypeIndex() == ClassDef.NO_INDEX) {
      ...
      throw new DexException(
          "Class with type index " + classDef.getTypeIndex() + " extends itself");
    } else {
