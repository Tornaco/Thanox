...
public final class BasicBlock implements LabeledItem {
  ...
  /**
   * {@code >= -1;} the primary / standard-flow / "default" successor, or {@code -1} if this block
   * has no successors (that is, it exits the function/method)
   */
   ...
   * @param primarySuccessor {@code >= -1;} the primary / standard-flow / "default" successor, or
   *     {@code -1} if this block has no successors (that is, it exits the function/method or is an
   *     unconditional throw)
   */
   ...
   * {@inheritDoc}
   *
   * <p>Return the identity hashcode of this instance. This is proper, since instances of this class
   * compare by identity (see {@link #equals}).
   */
  @Override
  public int hashCode() {
   ...
   *
   * @return {@code >= -1;} the primary successor, or {@code -1} if this block has no successors at
   *     all
   */
  ...
  /**
   * Gets the secondary successor of this block. It is only valid to call this method on blocks that
  ...
  /**
   * Gets the first instruction of this block. This is just a convenient shorthand for {@code
   * getInsns().get(0)}.
   *
  ...
  /**
   * Gets the last instruction of this block. This is just a convenient shorthand for {@code
   * getInsns().getLast()}.
   *
  ...
  /**
   * Returns whether this block might throw an exception. This is just a convenient shorthand for
   * {@code getLastInsn().canThrow()}.
   *
  ...
  /**
   * Returns whether this block has any associated exception handlers. This is just a shorthand for
   * inspecting the last instruction in the block to see if it could throw, and if so, whether it in
   * fact has any associated handlers.
   *
  ...
  /**
   * Returns the exception handler types associated with this block, if any. This is just a
   * shorthand for inspecting the last instruction in the block to see if it could throw, and if so,
   * grabbing the catch list out of it. If not, this returns an empty list (not {@code null}).
   *
   * @return {@code non-null;} the exception handler types associated with this block
   */
  ...
  /**
   * Returns an instance that is identical to this one, except that the registers in each
   * instruction are offset by the given amount.
   *
