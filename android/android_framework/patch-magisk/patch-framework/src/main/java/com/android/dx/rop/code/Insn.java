...
/**
 * A register-based instruction. An instruction is the combination of an opcode (which specifies
 * operation and source/result types), a list of actual sources and result registers/values, and
...
public abstract class Insn implements ToHuman {
   ...
   * {@inheritDoc}
   *
   * <p>Instances of this class compare by identity. That is, {@code x.equals(y)} is only true if
   * {@code x == y}.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>This implementation returns the identity hashcode of this instance. This is proper, since
  ...
  /**
   * Gets the result spec, if any. A return value of {@code null} means this instruction returns
   * nothing.
   *
  ...
  /**
   * Gets the spec of a local variable assignment that occurs at this instruction, or null if no
   * local variable assignment occurs. This may be the result register, or for {@code mark-local}
   * insns it may be the source.
   *
  ...
  /**
   * Gets whether this instruction can possibly throw an exception. This is just a convenient
   * wrapper for {@code getOpcode().canThrow()}.
   *
  ...
  /**
   * Gets the list of possibly-caught exceptions. This returns {@link StdTypeList#EMPTY} if this
   * instruction has no handlers, which can be <i>either</i> if this instruction can't possibly
   * throw or if it merely doesn't handle any of its possible exceptions. To determine whether this
  ...
  /**
   * Returns an instance that is just like this one, except that it has a catch list with the given
   * item appended to the end. This method throws an exception if this instance can't possibly
   * throw. To determine whether this instruction can throw, use {@link #canThrow}.
   *
  ...
  /**
   * Returns an instance that is just like this one, except that, if possible, the insn is converted
   * into a version in which a source (if it is a constant) is represented directly rather than as a
   * register reference. {@code this} is returned in cases where the translation is not possible.
   *
   * @return {@code non-null;} an appropriately-constructed instance
   */
  ...
  /**
   * Compares Insn contents, since {@code Insn.equals()} is defined to be an identity compare.
   * Insn's are {@code contentEquals()} if they have the same opcode, registers, source position,
   * and other metadata.
   *
  ...
  /**
   * Returns the string form of this instance, with the given bit added in the standard location for
   * an inline argument.
   *
  ...
  /**
   * Returns the human string form of this instance, with the given bit added in the standard
   * location for an inline argument.
   *
   * @param extra {@code null-ok;} the inline argument string
   * @return {@code non-null;} the human string form
   */
