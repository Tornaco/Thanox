 ...
 *
 * <p><b>Note:</b> Opcode descriptions use a rough pseudocode. {@code r} is the result register,
 * {@code x} is the first argument, {@code y} is the second argument, and {@code z} is the third
 * argument. The expression which describes the operation uses Java-ish syntax but is preceded by
 * type indicators for each of the values.
 */
public final class RegOps {
  ...
  /**
   * {@code T: Throwable; r: T :: r = caught_exception}. <b>Note:</b> This opcode should only ever
   * be used in the first instruction of a block, and such blocks must be the start of an exception
   * handler.
   */
  ...
  /** {@code T: int or Object; x,y: T :: if (x == y) goto label} */
  ...
  /** {@code T: any integral type; r,x: T; y: int :: r = x >> y} (signed right-shift) */
  public static final int SHR = 24;

  /** {@code T: any integral type; r,x: T; y: int :: r = x >>> y} (unsigned right-shift) */
  ...
  /**
   * {@code T: any numeric type; r: int; x,y: T :: r = (x == y) ? 0 : (x > y) ? 1 : -1} (Java-style
   * "cmpl" where a NaN is considered "less than" all other values; also used for integral
   * comparisons)
   */
  public static final int CMPL = 27;

  /**
   * {@code T: any floating point type; r: int; x,y: T :: r = (x == y) ? 0 : (x < y) ? -1 : 1}
   * (Java-style "cmpg" where a NaN is considered "greater than" all other values)
   */
  public static final int CMPG = 28;

  /**
   * {@code T: any numeric type; U: any numeric type; r: T; x: U :: r = (T) x} (numeric type
   * conversion between the four "real" numeric types)
   */
  ...
  /** {@code T: any array type; r: T; x: int; v0..vx: T :: r = new T[x] {v0, ..., vx}} */
  public static final int FILLED_NEW_ARRAY = 42;

  /** {@code T: any object type; x: Object :: (T) x} (can throw {@code ClassCastException}) */
  ...
  /** {@code T: any type; r: T; x: Object; f: instance field spec of type T :: r = x.f} */
  public static final int GET_FIELD = 45;

  /** {@code T: any type; r: T; f: static field spec of type T :: r = f} */
  public static final int GET_STATIC = 46;

  /** {@code T: any type; x: T; y: Object; f: instance field spec of type T :: y.f = x} */
  ...
  /**
   * {@code Tr, T0, T1...: any types; r: Tr; m: static method spec; y0: T0; y1: T1 ... :: r = m(y0,
   * y1, ...)} (call static method)
   */
  public static final int INVOKE_STATIC = 49;

  /**
   * {@code Tr, T0, T1...: any types; r: Tr; x: Object; m: instance method spec; y0: T0; y1: T1 ...
   * :: r = x.m(y0, y1, ...)} (call normal virtual method)
   */
  public static final int INVOKE_VIRTUAL = 50;

  /**
   * {@code Tr, T0, T1...: any types; r: Tr; x: Object; m: instance method spec; y0: T0; y1: T1 ...
   * :: r = x.m(y0, y1, ...)} (call superclass virtual method)
   */
  public static final int INVOKE_SUPER = 51;

  /**
   * {@code Tr, T0, T1...: any types; r: Tr; x: Object; m: instance method spec; y0: T0; y1: T1 ...
   * :: r = x.m(y0, y1, ...)} (call direct/special method)
   */
  public static final int INVOKE_DIRECT = 52;

  /**
   * {@code Tr, T0, T1...: any types; r: Tr; x: Object; m: interface (instance) method spec; y0: T0;
   * y1: T1 ... :: r = x.m(y0, y1, ...)} (call interface method)
   */
  public static final int INVOKE_INTERFACE = 53;

  /**
   * {@code T0: any type; name: local variable name :: mark(name,T0)} (mark beginning or end of
   * local variable name)
   */
  public static final int MARK_LOCAL = 54;

  /**
   * {@code T: Any type; r: T :: r = return_type}. <b>Note:</b> This opcode should only ever be used
   * in the first instruction of a block following an invoke-*.
   */
  public static final int MOVE_RESULT = 55;

  /**
   * {@code T: Any type; r: T :: r = return_type}. <b>Note:</b> This opcode should only ever be used
   * in the first instruction of a block following a non-invoke throwing insn
   */
  ...
  /**
   * {@code Tr, T0, T1...: any types; r: Tr; x: java.lang.invoke.MethodHandle; m: signature
   * polymorphic method spec; y0: T0; y1: T1 ... :: r = x.m(y0, y1, ...)} (call signature
   * polymorphic method)
   */
  ...
  /**
   * Given an IF_* RegOp, returns the right-to-left flipped version. For example, IF_GT becomes
   * IF_LT.
   *
