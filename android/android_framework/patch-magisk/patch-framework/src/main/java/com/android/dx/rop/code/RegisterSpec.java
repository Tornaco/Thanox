...
/**
 * Combination of a register number and a type, used as the sources and destinations of
...
public final class RegisterSpec implements TypeBearer, ToHuman, Comparable<RegisterSpec> {
   ...
   * @param type {@code non-null;} the type (or possibly actual value) which is loaded from or
   *     stored to the indicated register
  ...
  /**
   * Returns an instance for the given register number and type, with no variable info. This method
   * is allowed to return shared instances (but doesn't necessarily do so).
   *
   * @param reg {@code >= 0;} the register number
   * @param type {@code non-null;} the type (or possibly actual value) which is loaded from or
   *     stored to the indicated register
  ...
  /**
   * Returns an instance for the given register number, type, and variable info. This method is
   * allowed to return shared instances (but doesn't necessarily do so).
   *
   * @param reg {@code >= 0;} the register number
   * @param type {@code non-null;} the type (or possibly actual value) which is loaded from or
   *     stored to the indicated register
   ...
   * @param type {@code non-null;} the type (or possibly actual value) which is loaded from or
   *     stored to the indicated register
   * @param local {@code null-ok;} the associated variable info or null for none
   * @return {@code non-null;} an appropriately-constructed instance
   */
  ...
  /**
   * Helper for {@link #hashCode} and {@link
   * ForComparison#hashCode}, which actually does the
   * calculation.
   *
  ...
  /**
   * Like {@code equals}, but only consider the simple types of the registers. That is, this
   * compares {@code getType()} on the types to ignore whatever arbitrary extra stuff might be
   * carried around by an outer {@link TypeBearer}.
   *
   * @param other {@code null-ok;} spec to compare to
   * @return {@code true} iff {@code this} and {@code other} are equal in the stated way
   */
  ...
  /**
   * Like {@link #equalsUsingSimpleType} but ignoring the register number. This is useful to
   * determine if two instances refer to the "same" local variable.
   *
   * @param other {@code null-ok;} spec to compare to
   * @return {@code true} iff {@code this} and {@code other} are equal in the stated way
   */
  ...
  /**
   * Helper for {@link #equals} and {@link
   * ForComparison#equals}, which actually does the test.
   *
  ...
  /**
   * Compares by (in priority order) register number, unwrapped type (that is types not {@link
   * TypeBearer}s, and local info.
   *
  ...
  /**
   * Gets the type (or actual value) which is loaded from or stored to the register associated with
   * this instance.
   *
   ...
   *
   * @return {@code null-ok;} the variable info, or {@code null} if this instance has none
   */
  ...
  /**
   * Gets the next available register number after the one in this instance. This is equal to the
   * register number plus the width (category) of the type used. Among other things, this may also
   * be used to determine the minimum required register count implied by this instance.
   *
  ...
  /**
   * Gets the category of this instance's type. This is just a convenient shorthand for {@code
   * getType().getCategory()}.
   *
  ...
  /**
   * Gets whether this instance's type is category 1. This is just a convenient shorthand for {@code
   * getType().isCategory1()}.
   *
  ...
  /**
   * Gets whether this instance's type is category 2. This is just a convenient shorthand for {@code
   * getType().isCategory2()}.
   *
   ...
   *   <li>If the register numbers don't match, then the intersection is {@code null}. Otherwise,
   *       the register number of the intersection is the same as the one in the two instances.
   *   <li>If the types returned by {@code getType()} are not {@code equals()}, then the
   *       intersection is null.
   *   <li>If the type bearers returned by {@code getTypeBearer()} are {@code equals()}, then the
   *       intersection's type bearer is the one from this instance. Otherwise, the intersection's
   *       type bearer is the {@code getType()} of this instance.
   *   <li>If the locals are {@code equals()}, then the local info of the intersection is the local
   *       info of this instance. Otherwise, the local info of the intersection is {@code null}.
   * </ul>
   *
   * @param other {@code null-ok;} instance to intersect with (or {@code null})
   * @param localPrimary whether local variables are primary to the intersection; if {@code true},
   *     then the only non-null results occur when registers being intersected have equal local
  ...
  public RegisterSpec intersect(RegisterSpec other, boolean localPrimary) {
    ...
    return (resultLocal == null)
        ? make(reg, resultTypeBearer)
        : make(reg, resultTypeBearer, resultLocal);
  }

  /**
   * Returns an instance that is identical to this one, except that the register number is replaced
   * by the given one.
   *
  ...
  /**
   * Returns an instance that is identical to this one, except that the type is replaced by the
   * given one.
   *
  ...
  /**
   * Returns an instance that is identical to this one, except that the register number is offset by
   * the given amount.
   *
  ...
  /**
   * Returns an instance that is identical to this one, except that the type bearer is replaced by
   * the actual underlying type (thereby stripping off non-type information) with any initialization
   * information stripped away as well.
   *
   * @return {@code non-null;} an appropriately-constructed instance
   */
  ...
  /**
   * Returns an instance that is identical to this one except that the local variable is as
  ...
  /**
   * Holder of register spec data for the purposes of comparison (so that {@code RegisterSpec}
   * itself can still keep {@code final} instance variables.
   */
