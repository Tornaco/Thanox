...
/**
 * Instruction which contains an explicit reference to a constant and which might throw an
 * exception.
 */
public final class ThrowingCstInsn extends CstInsn {
  ...
  public Insn withAddedCatch(Type type) {
    return new ThrowingCstInsn(
        getOpcode(), getPosition(), getSources(), catches.withAddedType(type), getConstant());
  }
  ...
  public Insn withRegisterOffset(int delta) {
    return new ThrowingCstInsn(
        getOpcode(), getPosition(), getSources().withOffset(delta), catches, getConstant());
  }
