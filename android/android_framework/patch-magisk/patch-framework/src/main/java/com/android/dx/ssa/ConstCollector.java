...
/**
 * Collects constants that are used more than once at the top of the method block. This increases
 * register usage by about 5% but decreases insn size by about 3%.
 */
public class ConstCollector {
  ...
  /**
   * Also collect string consts, although they can throw exceptions. This is off now because it just
   * doesn't seem to gain a whole lot. TODO if you turn this on, you must change
   * SsaInsn.hasSideEffect() to return false for const-string insns whose exceptions are not caught
   * in the current method.
   */
  private static final boolean COLLECT_STRINGS = false;

  /**
   * If true, allow one local var to be involved with a collected const. Turned off because it
   * mostly just inserts more moves.
   */
