...
/**
 * Simple intraprocedural escape analysis. Finds new arrays that don't escape the method they are
 * created in and replaces the array values with registers.
 */
public class EscapeAnalysis {
  ...
  /**
   * Performs escape analysis on a method. Finds scalar replaceable arrays and replaces them with
  ...
  /**
   * Finds the index in the lattice for a particular register. Returns the size of the lattice if
   ...
   * @return {@code non-null;} the instruction that produces the result for the move
   */
   ...
   *
   * @param insn {@code non-null;} an instruction that must always be followed by a move result
  ...
  /**
   * Creates a link in the lattice between two EscapeSets due to a put instruction. The object being
   * put is the child and the object being put into is the parent. A child set must always have an
   * escape state at least as high as its parent.
   *
   * @param parentSet {@code non-null;} the EscapeSet for the object being put into
  ...
  /**
   * Merges all links in the lattice among two EscapeSets. On return, the newNode will have its old
   * links as well as all links from the oldNode. The oldNode has all its links removed.
   *
  ...
  /**
   * Determine the origin of a move result pseudo instruction that generates an object. Creates a
   * new EscapeSet for the new object accordingly.
   *
   * @param insn {@code non-null;} move result pseudo instruction to process
   * @return {@code non-null;} an EscapeSet for the object referred to by the move result pseudo
   *     instruction
   */
  ...
  /**
   * Handles phi uses of new objects. Will merge together the sources of a phi into a single
   * EscapeSet. Adds the result of the phi to the worklist so its uses can be followed.
   *
   * @param use {@code non-null;} phi use being processed
   * @param escSet {@code non-null;} EscapeSet for the object
   * @param regWorklist {@code non-null;} worklist of instructions left to process for this object
   */
  ...
  /**
   * Handles non-phi uses of new objects. Checks to see how instruction is used and updates the
   * escape state accordingly.
   *
  ...
  private void processUse(
      RegisterSpec def, SsaInsn use, EscapeSet escSet, ArrayList<RegisterSpec> regWorklist) {
  ...
  /**
   * Replaces the instructions that define an array with equivalent registers. For each entry in the
   * array, a register is created, initialized to zero. A mapping between this register and the
   * corresponding array index is added.
   *
   * @param def {@code non-null;} move result instruction for array
   * @param prev {@code non-null;} instruction for instantiating new array
   * @param length size of the new array
   * @param newRegs {@code non-null;} mapping of array indices to new registers to be populated
   */
  ...
  /**
   * Replaces the use for a scalar replaceable array. Gets and puts become move instructions, and
   * array lengths and fills are handled. Can also identify ArrayIndexOutOfBounds exceptions and
   * throw them if detected.
   *
   * @param use {@code non-null;} move result instruction for array
   * @param prev {@code non-null;} instruction for instantiating new array
   * @param newRegs {@code non-null;} mapping of array indices to new registers
   * @param deletedInsns {@code non-null;} set of instructions marked for deletion
   */
  private void replaceUse(
      SsaInsn use, SsaInsn prev, ArrayList<RegisterSpec> newRegs, HashSet<SsaInsn> deletedInsns) {
    int index;
    ...
    switch (use.getOpcode().getOpcode()) {
      ...
      case RegOps.ARRAY_LENGTH:
        ...
        next = getMoveForInsn(use);
        insertPlainInsnBefore(
            next, RegisterSpecList.EMPTY, next.getResult(), RegOps.CONST, (Constant) lengthReg);
        deletedInsns.add(next);
        break;
      ...
      case RegOps.FILL_ARRAY_DATA:
        ...
        for (int i = 0; i < length; i++) {
          ...
          insertPlainInsnBefore(
              use, RegisterSpecList.EMPTY, newFill, RegOps.CONST, constList.get(i));
  ...
  /**
   * Identifies extra moves added by scalar replacement and propagates the source of the move to any
   * users of the result.
   */
  ...
  private void run() {
    ssaMeth.forEachBlockDepthFirstDom(
        new SsaBasicBlock.Visitor() {
          @Override
  ...
  /**
   * Replaces instructions that trigger an ArrayIndexOutofBounds exception with an actual throw of
   * the exception.
   *
   ...
   * @param deletedInsns {@code non-null;} set of instructions marked for deletion
   */
  private void insertExceptionThrow(
      SsaInsn insn, RegisterSpec index, HashSet<SsaInsn> deletedInsns) {
    ...
    insertThrowingInsnBefore(
        newInsn2, RegisterSpecList.make(newReg, index), null, RegOps.INVOKE_DIRECT, newRef);
    ...
    newBlock3.replaceSuccessor(
        newBlock3.getPrimarySuccessorIndex(), ssaMeth.getExitBlock().getIndex());
    deletedInsns.add(newInsn3);
  }

  /**
   * Inserts a new PlainInsn before the given instruction. TODO: move this somewhere more
   * appropriate
   *
   ...
   */
  private void insertPlainInsnBefore(
      SsaInsn insn,
      RegisterSpecList newSources,
      RegisterSpec newResult,
      int newOpcode,
      Constant cst) {
  ...
  /**
   * Inserts a new ThrowingInsn before the given instruction. TODO: move this somewhere more
   * appropriate
   *
   * @param insn {@code non-null;} instruction to insert before
   * @param newSources {@code non-null;} sources of new instruction
   ...
   * @param cst {@code null-ok;} constant for new instruction, if any
   */
  private void insertThrowingInsnBefore(
      SsaInsn insn,
      RegisterSpecList newSources,
      RegisterSpec newResult,
      int newOpcode,
    ...
    if (cst == null) {
      ...
      newRopInsn =
          new ThrowingCstInsn(
              newRop, origRopInsn.getPosition(), newSources, StdTypeList.EMPTY, cst);
    }
  ...
  /**
   * Lattice values used to indicate escape state for an object. Analysis can only raise escape
   * state values, not lower them.
   *
   * <p>TOP - Used for objects that haven't been analyzed yet NONE - Object does not escape, and is
   * eligible for scalar replacement. METHOD - Object remains local to method, but can't be scalar
   * replaced. INTER - Object is passed between methods. (treated as globally escaping since this is
   * an intraprocedural analysis) GLOBAL - Object escapes globally.
   */
