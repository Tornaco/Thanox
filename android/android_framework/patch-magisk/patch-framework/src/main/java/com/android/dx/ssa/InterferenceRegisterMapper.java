...
/**
 * A register mapper that keeps track of the accumulated interference information for the registers
 * in the new namespace.
 *
 * <p>Please note that this mapper requires that the old namespace does not have variable register
 * widths/categories, and the new namespace does.
 */
public class InterferenceRegisterMapper extends BasicRegisterMapper {
  /**
   * Array of interference sets. ArrayList is indexed by new namespace and BitIntSet's are indexed
   * by old namespace. The list expands as needed and missing items are assumed to interfere with
   * nothing.
   *
   * <p>Bit sets are always used here, unlike elsewhere, because the max size of this matrix will be
   * (countSsaRegs * countRopRegs), which may grow to hundreds of K but not megabytes.
   */
  ...
  /**
   * Checks to see if old namespace reg {@code oldReg} interferes with what currently maps to {@code
   * newReg}.
   *
  ...
  /**
   * Checks to see if any of a set of old-namespace registers are pinned to the specified
   * new-namespace reg + category. Takes into account the category of the old-namespace registers.
   *
   * @param oldSpecs {@code non-null;} set of old-namespace regs
   * @param newReg {@code >= 0;} new-namespace register
   * @param targetCategory {@code 1..2;} the number of adjacent new-namespace registers (starting at
   *     ropReg) to consider
   * @return true if any of the old-namespace register have been mapped to the new-namespace
   *     register + category
   */
