...
public class SCCP {
  ...
  /**
   * Worklist for SSA edges that represent varying values. It makes the algorithm much faster if you
   * move all values to VARYING as fast as possible.
   */
  ...
  /**
   * Adds a SSA basic block to the CFG worklist if it's unexecuted, or to the CFG phi worklist if
  ...
  /**
   * Simulates a PHI node and set the lattice for the result to the appropriate value. Meet values:
   * TOP x anything = TOP VARYING x anything = VARYING CONSTANT x CONSTANT = CONSTANT if equal
   * constants, VARYING otherwise
   *
  ...
  /**
   * Replaces TypeBearers in source register specs with constant type bearers if possible. These are
   * then referenced in later optimization steps.
   */
