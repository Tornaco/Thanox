...
public final class SsaBasicBlock {
  /** {@code non-null;} comparator for instances of this class that just compares block labels */
  ...
  /** {@code non-null;} ordered successor list (same block may be listed more than once) */
  private IntList successorList;
  /** block list index of primary successor, or {@code -1} for no primary successor */
  private int primarySuccessor = -1;
  /**
   * the number of moves added to the end of the block during the phi-removal process. Retained for
   * subsequent move scheduling.
   */
  private int movesFromPhisAtEnd = 0;

  /**
   * the number of moves added to the beginning of the block during the phi-removal process.
   * Retained for subsequent move scheduling.
   */
  ...
  /**
   * Adds a phi insn to the beginning of this block. The result type of the phi will be set to void,
  ...
  /**
   * Adds a phi insn to the beginning of this block. This is to be used when the result type or
   * local-association can be determined at phi insert time.
   *
  ...
  /**
   * @return {@code non-null;} the (mutable) instruction list for this block, with phi insns at the
   *     beginning
   */
  ...
  /** @return {@code non-null;} ordered successor list, containing block indicies */
  public IntList getSuccessorList() {
    return successorList;
  }

  /**
   * @return {@code >= -1;} block index of primary successor or {@code -1} if no primary successor
   */
  ...
  /**
   * Attaches block to an exit block if necessary. If this block is not an exit predecessor or is
   * the exit block, this block does nothing. For use by {@link
   ...
   * before writes to that register. NOTE: caller is expected to returnSpareRegisters()!
   *
   * <p>TODO: See Briggs, et al "Practical Improvements to the Construction and Destruction of
   * Static Single Assignment Form" section 5. a) This can be done in three passes.
   *
  ...
  /**
   * Adds {@code regV} to the live-out list for this block. This is called by the liveness analyzer.
   *
  ...
  /**
   * Returns the set of live-in registers. Valid after register interference graph has been
