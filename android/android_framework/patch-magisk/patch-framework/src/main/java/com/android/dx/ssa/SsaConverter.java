...
public class SsaConverter {
  ...
  /**
   * Updates an SSA representation, placing phi functions and renaming all registers above a certain
   * threshold number.
   *
   * @param ssaMeth input
   ...
   *
   * <p>Converts CFG into "edge-split" form, such that each node either a unique successor or unique
   * predecessor.
   *
   * <p>
   *
   * <p>In addition, the SSA form we use enforces a further constraint, requiring each block with a
   * final instruction that returns a value to have a primary successor that has no other
   * predecessor. This ensures move statements can always be inserted correctly when phi statements
   * are removed.
   *
  ...
  /**
   * In ROP form, move-exception must occur as the first insn in a block immediately succeeding the
   * insn that could thrown an exception. We may need room to insert move insns later, so make sure
   * to split any block that starts with a move-exception such that there is a unique move-exception
   * block for each predecessor.
   *
   ...
   *   <li>the final instruction has any sources or results and the current successor block has more
   ...
   *
   * <p>Place Phi functions in appropriate locations.
   *
   * @param ssaMeth {@code non-null;} method to process. Modifications are made in-place.
  ...
  private static void placePhiFunctions(
      SsaMethod ssaMeth, LocalVariableInfo localInfo, int threshold) {
