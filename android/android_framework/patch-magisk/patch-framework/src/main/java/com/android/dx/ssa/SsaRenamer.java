 ...
 * <p>
 *
 * <p>See Appel algorithm 19.7
 *
 * <p>
 *
 * <p>Unlike the original algorithm presented in Appel, this renamer converts to a new flat
 ...
 * <p>
 *
 * <p>But, please note, the SSA Renamer's ignoring of the adjoining-register ROP representation
 * means that unaligned accesses to 64-bit registers are not supported. For example, you cannot do a
 ...
 * <p>
 *
 * <p>The implementation here, rather than keeping a single register version stack for the entire
...
public class SsaRenamer implements Runnable {
   ...
   * each dom parent for its children. The only sub-arrays that exist at any one time are the start
   * states for blocks yet to be processed by a {@code BlockRenamer} instance.
   */
   ...
   *
   * @param ssaMeth {@code non-null;} un-renamed SSA method that will be renamed.
   */
  ...
  /**
   * Returns true if this SSA register is below the specified threshold. Used when most code is
   * already in SSA form, and renaming is needed only for registers above a certain threshold.
   *
  ...
  private class BlockRenamer implements SsaInsn.Visitor {
    ...
    /**
     * {@code non-null;} indexed by old register name. The current top of the version stack as seen
     * by this block. It's initialized from the ending state of its dom parent, updated as the
     * block's instructions are processed, and then copied to each one of its dom children.
     */
    private final RegisterSpec[] currentMapping;

    /**
     * contains the set of moves we need to keep to preserve local var info. All other moves will be
     * deleted.
     */
     ...
     *       variable name, then every old-namespace register that maps to it should gain or lose
     *       its local variable name as well.
     ...
     * @param ssaReg {@code non-null;} an SSA register that has just been added to {@code
     *     currentMapping}
     */
     ...
     * {@inheritDoc}
     *
     * <p>Phi insns have their result registers renamed.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>Move insns are treated as a simple mapping operation, and will later be removed unless
     * they represent a local variable assignment. If they represent a local variable assignement,
     * they are preserved.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>All insns that are not move or phi insns have their source registers mapped ot the current
     * mapping. Their result registers are then renamed to a new SSA register which is then added to
    ...
    /**
     * Provides a register mapping between the old register space and the current renaming mapping.
     * The mapping is updated as the current block's instructions are processed.
     */
