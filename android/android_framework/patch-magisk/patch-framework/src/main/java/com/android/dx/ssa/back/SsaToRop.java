...
public class SsaToRop {
   ...
   * @param minimizeRegisters {@code true} if the converter should attempt to minimize the rop-form
   *     register count
   */
  ...
  private RopMethod convert() {
    ...
    RopMethod ropMethod =
        new RopMethod(
            convertBasicBlocks(), ssaMeth.blockIndexToRopLabel(ssaMeth.getEntryBlockIndex()));
  ...
  /**
   * Removes all blocks containing only GOTOs from the control flow. Although much of this work will
   * be done later when converting from rop to dex, not all simplification cases can be handled
   * there. Furthermore, any no-op block between the exit block and blocks containing the real
   * return or throw statements must be removed.
   */
  private void removeEmptyGotos() {
    final ArrayList<SsaBasicBlock> blocks = ssaMeth.getBlocks();

    ssaMeth.forEachBlockDepthFirst(
        false,
  ...
  /**
   * See Appel 19.6. To remove the phi instructions in an edge-split SSA representation we know we
   * can always insert a move in a predecessor block.
   */
  ...
  /**
   * Moves the parameter registers, which allocateRegisters() places at the bottom of the frame, up
   * to the top of the frame to match Dalvik calling convention.
   */
  ...
  /**
   * Validates that a basic block is a valid end predecessor. It must end in a RETURN or a THROW.
   * Throws a runtime exception on error.
   *
   ...
   *
   * @return a list of registers ordered by most-frequently-used to least-frequently-used. Each
   *     register is listed once and only once.
   */
  ...
  /**
   * Helper for {@link #removePhiFunctions}: PhiSuccessorUpdater for adding move instructions to
   * predecessors based on phi insns.
   */
