...
public class FixedSizeList extends MutabilityControl implements ToHuman {
   ...
   * {@inheritDoc}
   *
   * <p>This method will only work if every element of the list implements {@link ToHuman}.
   */
  @Override
  ...
  /**
   * Gets a customized human string for this instance. This method will only work if every element
   * of the list implements {@link ToHuman}.
   *
   * @param prefix {@code null-ok;} prefix for the start of the result
   * @param separator {@code null-ok;} separator to insert between each item
   * @param suffix {@code null-ok;} suffix for the end of the result
  ...
  /**
   * Shrinks this instance to fit, by removing any unset ({@code null}) elements, leaving the
   ...
   * never set; if you do that, this will throw {@code NullPointerException}. This method is
   * protected so that subclasses may offer a safe type-checked public interface to their clients.
   *
  ...
  /**
   * Gets the indicated element, allowing {@code null}s to be returned. This method is protected so
   * that subclasses may (optionally) offer a safe type-checked public interface to their clients.
   *
   * @param n {@code >= 0, < size();} which element
  ...
  /**
   * Sets the element at the given index, but without doing any type checks on the element. This
   * method is protected so that subclasses may offer a safe type-checked public interface to their
   * clients.
   *
   * @param n {@code >= 0, < size();} which element
  ...
  /**
   * Helper for {@link #toString} and {@link #toHuman}, which both of those call to pretty much do
   * everything.
   *
   * @param prefix {@code null-ok;} prefix for the start of the result
   * @param separator {@code null-ok;} separator to insert between each item
