...
public class LabeledList extends FixedSizeList {
   ...
   * @return {@code >= -1;} the index of the so-labelled item, or {@code -1} if none is found
   */
  ...
  /**
   * Gets an array containing all of the labels used in this instance, in order. The returned array
   * is freshly-allocated and so may be modified safely by the caller without impacting this
   * instance.
   *
   * @return {@code non-null;} ordered array of labels
   * @throws NullPointerException thrown if there are any {@code null} items in this instance
   */
  ...
  /**
   * Rebuilds the label-to-index mapping after a {@code shrinkToFit()}. Note: This assumes that the
   * labels that are in the list are the same, although the indicies may have changed.
   */
