...
/**
 * Class that takes a combined output destination and provides two output writers, one of which ends
 * up writing to the left column and one which goes on the right.
 */
public final class TwoColumnOutput {
  ...
  /**
   * Appends a newline to the given buffer via the given writer, but only if it isn't empty and
  ...
  /**
   * Flushes the output. If there are more lines of pending output in one column, then the other
   * column will get filled with blank lines.
   */
  public void flush() {
    try {
  ...
  /**
   * Outputs to the final destination as many full line pairs as there are in the pending output,
   * removing those lines from their respective buffers. This method terminates when at least one of
   * the two column buffers is empty.
   */
  ...
  /**
   * Flushes the left column buffer, printing it and clearing the buffer. If the buffer is already
  ...
  /**
   * Flushes the right column buffer, printing it and clearing the buffer. If the buffer is already
   * empty, this does nothing.
   */
